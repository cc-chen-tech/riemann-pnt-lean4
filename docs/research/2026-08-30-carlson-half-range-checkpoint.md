...
以下数值只对应该历史提交及当时源码指纹，不对应后来的集成树。
[历史机器可读验收记录](2026-08-30-carlson-checkpoint-validation.json) 包含：
...
**上述历史发布为草稿检查点 PR，不是 main 集成或全仓发布验收。** 原成果上的
`scripts/verify-baseline.sh` 运行在改接口前被主动中断，退出码 `130`；
它没有通过，也没有被分支范围的检查替代。

后续 `a799413579f00643b1145bf43bcd0ff7b8a103b9` 的全仓尝试（session 17049）
真实退出码为 `1`：缺失 `MathlibAux.ExponentialPolynomialFirstMoment.olean`，
其后门禁没有运行。见 [当时记录](https://github.com/cc-chen-tech/riemann-pnt-lean4/pull/504#issuecomment-5471977648)。
这不是原 152 契约的失败，也不是最终集成的成功。

2026-08-31 的隔离集成保全 main 的全部既有 roots，只移除本分支新增的一项
`LogMeanSquare` 重复注册。最终验收目前待完成；必须在固定候选 commit/tree 上
重新取得全仓 baseline、152 契约与公理审计、完整 Python 和清单检查的真实结果。
新证据外置绑定候选 commit/tree、实际 main 起止 SHA、命令、退出码和日志；
不能用本页历史数值填充。GitHub 无 CI 记录须单列为 absent，不记为通过。
是否已合并以 GitHub 的实际状态为准；实际 forcing 供应者仍未证明。

原成果记录保留在[历史验收 JSON](2026-08-30-carlson-half-range-verification.json)，
对应原成果提交 `64417646`；该 JSON 自身记录的起始 HEAD 是 precommit
`fdd50001` 加当时未提交实现，不能抹去这一区别。
复用时必须同时引用本检查点的源码版本与证明边界，
