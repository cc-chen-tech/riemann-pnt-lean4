...
后续 [全部双频率 adapter 与零差带求值](2026-08-30-mwkf-shifted-frequency-adapter.md) 给 PA18 的补集一个显式物理频率表达式，连同原两条删轴和统一截断尾。零差整带将 κχ 替换为 2γ，但保留 λ(t) 级长 Möbius Gram；这不证明其上界，也不授权逐单个 Poisson 频率做 reflection。
