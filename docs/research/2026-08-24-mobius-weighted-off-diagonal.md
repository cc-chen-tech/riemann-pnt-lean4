...
\(X^2/G\) to \(X^2/s\).  The required signed varying-conductor moment,
now reduced in the conductor variables to the total modulus \(s\) but
still jointly in \(s,\xi,h\delta\), and the short-product wedge remain unproved;
the latter is exactly \(x<2\).
Finally,
