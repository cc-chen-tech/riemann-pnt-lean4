...
变量变长就丢掉单位掩码。保留两者后，在下述声明的模型中，可以严格
控制一部分 gcd 原子；该模型的剩余账本集中到两个平滑变量同时被
大因子整除的交叠项。物理包是否满足全部前提仍须逐项适配。
...
实际删区的验收标准见[物理原子交接清单](2026-08-30-pr503-scope-and-physical-handoff.md)。
模型中某模态的消失，不是原始表达式对应项已被消掉的证据。
