...
`Classical.choice`、`Quot.sound`。这完成 `HJ-horizontal-explicit`。
...
这个七次幂比真实量级宽松很多，但没有消耗任何新的解析抵消。截至
2026-08-29，上述五步已经逐项形式化；实际 `F'/F` 的同高度积分满足
`HJ-horizontal-coarse`，两个存在常数始终独立。另有单独的极限定理证明

\[
 {1{,}100{,}000{,}000{,}000L^7\over e^L/L}\longrightarrow0,
\]

即该水平边确为 `o(e^L/L)`。完整类型 contract 与公理审计均通过。
...
这是比已证明七次幂更尖的可选目标，不再是式 (37) 闭合的必要门。
若以后希望改善常数或幂次，可以再证明（指数 `5` 可由更粗的固定指数
替代）
...
由已证明的 `HJ-horizontal-coarse` 及其极限，两条分别选择的水平项合计
为 `o(e^L/L)`。已证明的 `507e^L/L` 控制的是移动右边上的
...
并且不能由 `507e^L/L` 的绝对对数积分推出。后续
`2026-08-29-conrey-right-argument-math.md` 已采用更强的右半平面办法：
证明整条边上 `Re F>=3/10`，再由主值对数的 FTC 把该积分精确化为两个
端点 argument 之差，绝对值至多 `pi`。因此这个门现已闭合；下一步是
把它与两条水平项及 `(37-exact)` 正式拼接。
