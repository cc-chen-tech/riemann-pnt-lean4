...
同一个剥零因子的中心/球面估计以及精确 Borel--Caratheodory 界也已由
Lean 逐项核验。尚未完成的是 5.3 的因子盘支撑选高和带权主部积分；
因此完整水平积分仍不能标为完成。
...
`g` 同时满足 `HJ-factor-center`、`HJ-factor-sphere` 和精确的第一条
`HJ-regular-logderiv` 不等式。contract 核验这些端点只依赖标准公理
`propext`、`Classical.choice`、`Quot.sound`。其中进一步粗化到常数
`128` 的第二条不等式仍留给尺度整理步骤，不能把它冒充已经形式化。
