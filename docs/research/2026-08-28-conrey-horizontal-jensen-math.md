...
Borel--Caratheodory 界也已由 Lean 逐项核验。把这三个端点与独立的
`m_b<=J`、`delta_J<=delta` 合成为下面显示的 `V(J,delta_J)` 粗化版尚未
形式化；5.3 的因子盘支撑选高和带权主部积分也尚未完成。因此完整水平
积分仍不能标为完成。
...
不等式。contract 核验这些端点只依赖标准公理 `propext`、
`Classical.choice`、`Quot.sound`。尚未形式化的有两次后处理：第一是把
实际量同时替换成 `J,delta_J`，得到上面显示的 `V`；第二是把 Borel
几何因子粗化到常数 `128`。二者都不能冒充已完成端点。
