...
**实际 quotient 核的后续核查：** 该几何例子不是物理困难子区间。
[实际 Fourier 支撑与过渡式](2026-08-30-physical-quotient-support-transition.md)
证明 quotient 的第二次变换恢复原正区间，排除这里靠近零的 \(\lambda\)。
通用核的 (18) 仍然成立，但物理残余须在保留真实 \(r/R\) 权后定位。
...
但自然尺度过渡区与其余非零模式仍须保留。通用例子并不指定真实物理
核的困难子区间；后续实际支撑重组把平衡近对角定位于
\(r=s+w\)、\(|w|\asymp T^2\)，保留 \((s,w)=1\) 和全部原权。
下一步必须利用真实 Type 系数、Möbius 符号或跨模式抵消。
