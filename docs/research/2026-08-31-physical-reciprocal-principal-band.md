...
先去掉 1/φ，但不能直接把平方除数和单位除数相乘。精确恒等式是
\[
 \mu^2(c)1_{(c,D_0)=1}
 =\sum_{\substack{u^2\mid c\\(u,D_0)=1}}\mu(u)
   \sum_{\substack{v\mid D_0\\v\mid c/u^2}}\mu(v).
\]
这是先用 (c,D₀)=1 强制 (u,D₀)=1，再对 c/u² 展开单位掩码。
因此 (u,v)=1，才可合法写 c=u²vy；不漏掉或错误重计重叠素数。
若省去 (u,D₀)=1，c=4、D₀=2 的错误乘积展开会得 −1 而非 0。
取绝对值后可以丢弃 (u,D₀)=1 来求上界，RP11 第一项求和为
