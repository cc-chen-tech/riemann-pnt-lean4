...
令 `w=A-1/2≥2`，`J` 为辅助 `Y=2,R=0` 圆盘内完整重数的上界，
`K=128 max(V,1) outer/gap²≥0` 为同一正则因子的界。独立证明无权
积分不超过 `wK+πJ` 后，可以使用纯代数比较

\[
 (w^2/2)K+w\pi J-(wK+\pi J)
 =w(w-2)K/2+(w-1)\pi J\ge0.
\]

因此既有显式右端的 `1.1·10^12 L^7` 上界可复用。这里比较的是两个
显式非负上界，**绝不是从加权积分界反推无权积分界**。辅助阈值与
...
\[
 E_\eta(A;U,T)\ge
 \frac12\int_U^T\log\frac{t}{2\pi}\,dt
 -8(T-U)-w(K_H(b_-)+K_H(b_+))
 -2.2\cdot10^{12}L^7-\pi.
\]
...
`ConreyHorizontalArgument` 证明 `B₂≡1`、全高度标量核可积性、单根
`π` 界及非负有限完整重数的积分界。`ConreyV1HorizontalArgument`
对同一实际产品及圆盘构造 `D` 和完整有限支集 `P`，证明其重数和
精确等于 Jensen mass；同一抽取因子给出正则项及主部分解。
有限实值和通过逐项归纳建立可积性，避免通用扩展范数求和接口的
实例不匹配；没有引入完整奇异复核的可积性假设。

公开端点 `exists_conreyV1_horizontalArgument_le_coarse` 返回统一
`L0=40000+Creg+Cmass`。对于 `L≥L0`、`A+1≤base`、`base+1≤exp L`
与任意给定 `t∈[base,base+1]`，只要实际 V₁ 在 `[1/2,A]+it` 上非零，
就有无权辐角积分绝对值 `≤1.1·10^12 L^7`。阈值独立于 `L,base,t`，
高度不是重新选择的存在量。辅助 `Y=2,R=0` 不改变主均方的长 `Y`。

五个精确合约先因缺少目标定理失败；最终与既有加权水平积分及选高
均方合约组合构建通过（8861 jobs，exit 0）。五条新公开定理均仅
依赖 `propext`、`Classical.choice`、`Quot.sound`。Python 全量回归
546 passed；目标清单与依赖链检查通过。新端点 126 个本地模块的
import 闭包无 Zeta23，外部根仅 Mathlib。独立只读数学及代码审查
未发现问题，生产模块和合约均为默认 Lake roots。本次仅定向 Lean
