...
RP6 的平方／单位容斥修订使用 b3a758bcc01540aed5855bd3f1b10e4b7b660721，
旧稿未写明的互素条件不能省略。下文重新证明全 δ 的高导子适配，
不直接搬用旧固定 L 结论。
...
先把 χ 的零延拓单位条件并入 D_*=D₀ℓ，使用精确恒等式
\[
 \mu^2(C)1_{(C,D_*)=1}
 =\sum_{\substack{u^2\mid C\\(u,D_*)=1}}\mu(u)
   \sum_{\substack{v\mid D_*\\v\mid C/u^2}}\mu(v).
\]
保留 (u,D_*)=1 后才有 (u,v)=1，才能令 C=u²vy。
不能未经此限制把平方除数与单位除数的 lcm 当成乘积。
χ(u²v) 是固定模长≤1的标量；非 ℓ-unit 的 v 项由零延拓为零，
余下才是单位标量。取绝对值后丢弃 u 的限制只放大上界。
