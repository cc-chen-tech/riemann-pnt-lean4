...
| Explicit formula | `explicit_formula_von_mangoldt` is a `def ... : Prop` target | Replace the unconditional infinite `tsum` target with a truncated Perron/residue formula for `psi0`, then a principal-value limit | A concrete multiplicity-aware truncated first-order formula, Perron limit, trivial-zero limit, right horizontal decay, and a joint cofinal contour sequence are theorem-level; next control `-1<=Re(s)<=1+epsilon`, prove the moving-left remainder vanishes, and handle the nontrivial-zero principal-value limit | 1 |
