...
| Hardy quantitative extensions | Hardy's theorem and the Hardy--Littlewood linear lower bound are proved; the old Selberg split-energy endpoint is proved vacuous, while the replacement route separates the oscillatory `q=1` carrier from the noncarrier remainder and preserves the actual signed convolution square on the independent `hsmall` high range | Bound the carrier's explicit signed high-product tail and the noncarrier `L²` remainder for `hexcessive`; for `hsmall`, bound the signed convolution energy, actual truncation tail, and off-diagonal gap sum; then choose parameters jointly | Derive the Selberg `T log T` lower bound, then address the separately formulated Conrey percentage estimate | 7 (1 Selberg count target, 2 signed-moment targets, 3 in `HardyTheorem.Details`, 1 in `KnownResults`) |
