...
| Hardy quantitative extensions | Hardy's theorem and the Hardy--Littlewood linear lower bound are proved, including positive-ordinate odd and multiplicity-weighted forms | Close the two Selberg bad-window measure estimates | Derive the Selberg `T log T` lower bound, then address the separately formulated Conrey percentage estimate | 7 (1 Selberg count target, 2 signed-moment targets, 3 in `HardyTheorem.Details`, 1 in `KnownResults`) |
