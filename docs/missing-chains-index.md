...
| Explicit formula | `ExplicitFormulaTruncatedTarget` is proved for fixed `x`; the right Perron edge also has one uniform `C*m^5/W` bound at all natural `m>=2` | Uniformize the selected-height horizontal bound for `x>=2` and assemble at polynomial height; unrestricted real-`x` first-order Perron needs a distance-to-integer term | Extend the integer-sample formula to an RH-scale bound and then use monotonicity to cover real `x` | 0 |
