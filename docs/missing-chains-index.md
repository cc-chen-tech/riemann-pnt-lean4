...
| Hardy quantitative extensions | Hardy's theorem and the Hardy--Littlewood linear lower bound are proved; Selberg's fixed coprime-ray sum has an exact harmonic four-term expansion, and its complete-range single-log terms reduce to `log(a/b)` times the zero-order ray sum | Prove `1/log X` energy and reciprocal-gap savings by controlling non-diagonal rays, the double-log term, and truncation boundaries; use them to close `hexcessive`, then `hsmall` | Derive the Selberg `T log T` lower bound, then address the separately formulated Conrey percentage estimate | 7 (1 Selberg count target, 2 signed-moment targets, 3 in `HardyTheorem.Details`, 1 in `KnownResults`) |
