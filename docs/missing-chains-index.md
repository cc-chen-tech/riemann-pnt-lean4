...
| Explicit formula | `explicit_formula_von_mangoldt` is a `def ... : Prop` target | Replace the unconditional infinite `tsum` target with a truncated Perron/residue formula for `psi0`, then a principal-value limit | Perron, fixed-right-edge residues, right horizontal decay, trivial-zero simplicity/limit, exact moving-contour pole splitting, and a joint cofinal contour sequence are theorem-level; next control `-1<=Re(s)<=1+epsilon` and prove the moving-left-edge/remaining-pole limits | 1 |
