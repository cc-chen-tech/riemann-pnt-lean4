...
| Explicit formula | `explicit_formula_von_mangoldt` is a `def ... : Prop` target | Replace the unconditional infinite `tsum` target with a truncated Perron/residue formula for `psi0`, then a principal-value limit | A concrete multiplicity-aware truncated formula and an exact cofinal limit for `nontrivial-zero sum - contour remainder` are theorem-level. Gauss' digamma series and a logarithmic right-half-plane bound are now proved, and both complete moving far-left horizontal contour integrals vanish. Next control the moving left vertical edge and `ζ'/ζ` on `-1<=Re(s)<=1+epsilon`, then pass from the chosen sequence to arbitrary truncation heights | 1 |
