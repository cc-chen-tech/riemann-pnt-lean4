...
| Explicit formula | `explicit_formula_von_mangoldt` is a `def ... : Prop` target | Replace the unconditional infinite `tsum` target with a truncated Perron/residue formula for `psi0`, then a principal-value limit | Perron, fixed-right-edge residues, right horizontal decay, trivial-zero simplicity, and the multiplicity-aware trivial-residue limit are theorem-level; next control `-1<=Re(s)<=1+epsilon`, split the moving pole finsets, and handle the left-edge/nontrivial-zero limits | 1 |
