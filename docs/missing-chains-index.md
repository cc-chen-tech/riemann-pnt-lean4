...
| Explicit formula | `explicit_formula_von_mangoldt` is a symmetric-height `Tendsto` target, but its zero sum is still unweighted | Replace the final approximation by a multiplicity-aware symmetric-height sum, then promote the proved selected-sequence limit to all truncation heights | The exact finite moving-rectangle identity, all boundary decay, vanishing of the complete contour remainder, and the multiplicity-weighted zero-sum limit are theorem-level along one strictly increasing cofinal good-height sequence. Align the final target with zero multiplicities and control zero contributions between consecutive selected heights; the selected-sequence theorem is not yet a full principal-value limit | 1 |
