...
| Explicit formula | `explicit_formula_von_mangoldt` is a `def ... : Prop` target | Replace the unconditional infinite `tsum` target with a truncated Perron/residue formula for `psi0`, then a principal-value limit | A concrete multiplicity-aware truncated formula and an exact cofinal limit for `nontrivial-zero sum - contour remainder` are theorem-level; next prove the remainder vanishes, requiring `ζ'/ζ` bounds on `-1<=Re(s)<=1+epsilon` and quantitative Gamma/digamma control on both the moving left edge and the far-left horizontal segments, then pass from the chosen sequence to arbitrary truncation heights | 1 |
