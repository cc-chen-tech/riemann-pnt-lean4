...
| Explicit formula | `explicit_formula_von_mangoldt` is a `def ... : Prop` target | Replace the unconditional infinite `tsum` target with a truncated Perron/residue formula for `psi0`, then a principal-value limit | A concrete multiplicity-aware truncated formula and an exact cofinal limit for `nontrivial-zero sum - contour remainder` are theorem-level. Both complete moving far-left horizontal pieces and the complete moving left vertical edge now vanish along a linearly controlled good-height sequence. Next control `ζ'/ζ` on the horizontal central band `-epsilon<=Re(s)<=1+epsilon`, then pass from the chosen sequence to arbitrary truncation heights | 1 |
