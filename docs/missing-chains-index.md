...
| Hardy quantitative extensions | Hardy's theorem and the Hardy--Littlewood linear lower bound are proved; Selberg's `hexcessive` side is reduced to one explicit reduced-pair split energy, while the independent `hsmall` high range preserves the actual signed convolution square without a fiber-cardinality loss | Prove the reduced-pair split-energy bound for `hexcessive`; for `hsmall`, bound the signed convolution energy, actual truncation tail, and off-diagonal gap sum; then choose parameters jointly | Derive the Selberg `T log T` lower bound, then address the separately formulated Conrey percentage estimate | 7 (1 Selberg count target, 2 signed-moment targets, 3 in `HardyTheorem.Details`, 1 in `KnownResults`) |
