...
| Explicit formula | `explicit_formula_von_mangoldt` is a `def ... : Prop` target | Complete a truncated Perron/residue formula for `psi0`, then justify the principal-value limit | The Perron starting-line limit, finite multiplicity-aware residue identity, moving far-left horizontal pieces, moving left vertical edge, fixed right horizontal pieces, and inner zero-free horizontal pieces are theorem-level. Next prove a quantitative good-height `zeta'/zeta` bound on the remaining central horizontal band, combine all edge limits, and pass beyond the selected cofinal sequence | 1 |
