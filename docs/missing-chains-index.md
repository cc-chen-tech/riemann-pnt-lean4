...
| Explicit formula | `explicit_formula_von_mangoldt` is a `def ... : Prop` target | Replace the unconditional infinite `tsum` target with a truncated Perron/residue formula for `psi0`, then a principal-value limit | The first-order Perron limit and complete fixed-right-edge finite residue shift are theorem-level; next make the horizontal edges vanish and either evaluate the fixed `Re(s)=-1` left-line limit or move the left edge through the trivial zeros | 1 |
