...
| Explicit formula | `explicit_formula_von_mangoldt` is a `def ... : Prop` target | Replace the unconditional infinite `tsum` target with a truncated Perron/residue formula for `psi0`, then a principal-value limit | Prove the Perron half-jump identity and rectangle contour-error bounds; `psi0`, finite zero sums, and a strictly increasing good-height sequence tending to infinity are now theorem-level | 1 |
