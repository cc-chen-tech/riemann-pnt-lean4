...
| --- | --- | --- | --- | --- |
| Quantitative zero-free region | `classical_zero_free_region` and `vinogradov_korobov_zero_free_region` are `def ... : Prop` targets | Add zeta-specific growth/log-derivative estimates; do not cite compact zero-free region as the classical `c / log |t|` result | Use the proved meromorphic `ζ'/ζ` API to prove quantitative local logarithmic-derivative and Borel-Caratheodory/Jensen estimates; conditional 3-4-1 assembly and compact patching are already proved | 2 |
