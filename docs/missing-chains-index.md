...
| Explicit formula | `explicit_formula_von_mangoldt` is a `def ... : Prop` target | Replace the unconditional infinite `tsum` target with a truncated Perron/residue formula for `psi0`, then a principal-value limit | The first-order Perron starting-line limit is theorem-level, and the inner zero-free portions of both horizontal edges now tend to zero from a proved full-norm `O(log^2 |t|)` estimate. Control the remaining horizontal and left-edge pieces and combine them with the finite residue identity | 1 |
