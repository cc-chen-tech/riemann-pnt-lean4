...
| Hardy quantitative extensions | Hardy's theorem and the Hardy--Littlewood linear lower bound are proved; Selberg's fixed coprime-ray sum now has an exact harmonic four-term log expansion | Bound the four signed arithmetic sums strongly enough to close `hsmall` or `hexcessive` | Derive the Selberg `T log T` lower bound, then address the separately formulated Conrey percentage estimate | 7 (1 Selberg count target, 2 signed-moment targets, 3 in `HardyTheorem.Details`, 1 in `KnownResults`) |
