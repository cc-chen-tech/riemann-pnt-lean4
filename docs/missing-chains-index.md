...
| Explicit formula | `explicit_formula_von_mangoldt` is a `def ... : Prop` target | Replace the unconditional infinite `tsum` target with a truncated Perron/residue formula for `psi0`, then a principal-value limit | The Perron limit, complete fixed-right-edge residue shift, and `Re(s)>=1+epsilon` horizontal decay are theorem-level; next control `-1<=Re(s)<=1+epsilon` on good heights and evaluate the fixed left line or move it through the trivial zeros | 1 |
