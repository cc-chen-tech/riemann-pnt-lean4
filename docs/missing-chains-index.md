...
| --- | --- | --- | --- | --- |
| Quantitative zero-free region | `classical_zero_free_region` and `vinogradov_korobov_zero_free_region` are `def ... : Prop` targets | Add zeta-specific divisor/growth/log-derivative estimates; do not cite compact zero-free region as the classical `c / log |t|` result | Use the proved zeta meromorphicity to prove pole-order/divisor and logarithmic-derivative estimates; conditional 3-4-1 assembly and compact patching are already proved | 2 |
