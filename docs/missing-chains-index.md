...
| Hardy quantitative extensions | Hardy's theorem and the Hardy--Littlewood linear lower bound are proved; Selberg's complete ray has exact taper factorization, boundary scales lie in an explicit harmonic tail, and the global model budget has an exact energy-plus-gap decomposition that diagnoses why a positive absolute-value majorant is too coarse | Prove the direct Hermitian short-window kernel budget by separating stationary and nonstationary frequencies; use it to close `hexcessive`, then close the independent `hsmall` estimate | Derive the Selberg `T log T` lower bound, then address the separately formulated Conrey percentage estimate | 7 (1 Selberg count target, 2 signed-moment targets, 3 in `HardyTheorem.Details`, 1 in `KnownResults`) |
