...
| Explicit formula | `explicit_formula_von_mangoldt` is a `def ... : Prop` target | Replace the unconditional infinite `tsum` target with a truncated Perron/residue formula for `psi0`, then a principal-value limit | A concrete multiplicity-aware truncated formula and an exact cofinal limit for `nontrivial-zero sum - contour remainder` are theorem-level; the far-left horizontal Euler-product terms now provably vanish, leaving explicit Gamma/digamma integrals there. Next control those Gamma terms, the moving left vertical edge, and `ζ'/ζ` on `-1<=Re(s)<=1+epsilon`, then pass from the chosen sequence to arbitrary truncation heights | 1 |
