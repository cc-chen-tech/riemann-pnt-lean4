...
| Hardy quantitative extensions | Hardy's theorem and the Hardy--Littlewood linear lower bound are proved; the old Selberg split-energy endpoint is proved vacuous.  The replacement route separates the oscillatory `q=1` carrier, proves its coefficient is at most `1 + log X`, and reduces its window cost to `(1 + log X) * 32 / log T`.  Deleting the carrier improves local separation, reindexes the surviving energy exactly by positive coprime pairs with `(1,1)` removed, and now gives a direct mean-square bound for the actual Hardy-phase noncarrier short model; the independent `hsmall` high range preserves the actual signed convolution square | Prove the deleted coprime-pair coefficient and local-separation energies are uniformly small enough for `hexcessive`; for `hsmall`, bound the signed convolution energy, actual truncation tail, and off-diagonal gap sum; then choose parameters jointly | Derive the Selberg `T log T` lower bound, then address the separately formulated Conrey percentage estimate | 7 (1 Selberg count target, 2 signed-moment targets, 3 in `HardyTheorem.Details`, 1 in `KnownResults`) |
