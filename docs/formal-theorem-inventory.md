...
formalization, supplies numerically explicit remainder constants, or proves RH.
...
- `analyticOrderAt_riemannZeta_ne_top_of_ne_one`,
  ...
  `exists_eventually_atRight_norm_neg_logDeriv_...` and
  `exists_eventually_atRight_re_neg_deriv_div_...` package the signed norm
  regular-part estimate and its real-part consequence as eventual statements on
  the right-neighborhood of `beta`.
...
- `norm_logDeriv_le_two_mul_div_of_analyticOnNhd_nonzero_re_log_bound` gives the
  moving-interior-point form: when `dist(z,c)+rho<=R/2`, Cauchy's estimate on
  the remaining radius `rho` proves `‖g'/g(z)‖<=2M/rho`.
...
- `norm_logDeriv_le_two_mul_max_add_log_three_div_of_sphere_log_norm_le`
  combines the same boundary and center inputs with the moving-point Cauchy
  estimate, yielding `‖g'/g(z)‖<=2 max(B+log 3) 1/rho` whenever
  `dist(z,c)+rho<=R/2`.
- `exists_good_radius_log_norm_and_logDeriv_riemannZeta_factor_le_jensen_bound`
  ...
  Borel/Cauchy bound at `2+it` and throughout the strict interior of its
  guaranteed half-disk; it does
  not yet cover the whole `1<=sigma<=2` band or prove the zero-candidate
...
### Completed de la Vallee Poussin-form prime-counting error

- `PrimeNumberTheorem.exists_abs_primeCounting_sub_logIntegral_le_exp_neg_sqrt_log`
  proves that some `c > 0`, `C >= 0`, and threshold `X` satisfy
  `|pi(x)-Li(x)| <= C*x*exp(-c*sqrt(log x))` for every real `x >= X`.
- The proof is in `PrimeNumberTheorem/ClassicalPrimeCountingError.lean`; it
  transfers the estimate to `theta`, proves the quantitative Abel integral
  bound by splitting at `sqrt x`, and assembles the exact partial-summation
  identity.  It does not provide numerical values for `c`, `C`, or `X`.
...
> **Externally closed (2026-08, verified):** this target is implied by the
> independently built and kernel-checked `Zeta23.thmB₀_mult_cumulative`
> (Anthropic `zeta-23-lean` Theorem B: at least 2/3 of zeta zeros are simple
> and on the critical line; `#print axioms` = `[propext, Classical.choice,
> Quot.sound]`). The in-repo definitional bridge (toolchain v4.33 + simple-
> subset-of-odd lemma) is pending; see
...
- `conrey_40_percent_zeros_on_critical_line_target`
  ...
  **Externally closed (2026-08, verified):** follows from the Zeta23-implied
  `selberg_odd_zero_proportion_target` through the repo's existing
  `selberg_zero_proportion_target_of_odd` and
