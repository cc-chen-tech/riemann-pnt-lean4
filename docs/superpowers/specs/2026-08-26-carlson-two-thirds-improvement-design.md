...
2026-08-30 更新：用户要求的某个显式正节省已由半长两尺度路线取得；
无条件 Lean 全局定理的参数是 `delta=1/400,B=6`，包含闭实部阈值。
见 [结果与完整账本](../../research/2026-08-30-carlson-half-range-density-result.md)。
以下较强 `delta=5/64` 的原设计是独立的纸面目标，不是本次形式化完成声明。
