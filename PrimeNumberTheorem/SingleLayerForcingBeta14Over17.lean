...
part `β > 14/17` and window exponent `λ > 0`, the coherent F_R/F_L budget
...
theorem powerGrowth_logGap_contradiction
  ...
  have hbound : ∀ᶠ X in atTop, X ≤ (B / A) ^ (2 / d) := by
    ...
    have hX : X ^ (d / 2) ≤ B / A := by
      ...
      have hden : 0 < A * X ^ (d / 2) := mul_pos hA hXpos2
      have hmain : (A * X ^ (d / 2)) * (X ^ (d / 2)) ≤ B * X ^ (d / 2) := by
        ...
        rw [← hXd]
        exact hupper
      exact (le_div_iff₀' hden).mpr (by simpa [mul_comm] using hmain)
...
`σ = 2/3`, `T = X^(λ(1-β))` with the exponent gap beats the Carlson upper
bound once the exponent comparison `λ(1-β)q(σ) < 2λ(β-σ) − λ(1-β)q(σ)`
holds (for `σ = 2/3`, `λ = 1` this is exactly `β > 14/17`). -/
theorem singleLayerForcing_carlson_contradiction
    {σ β λ c k : ℝ}
    (hσ : 1 / 2 < σ) (hσ1 : σ < 1) (hσβ : σ ≤ β) (hβ1 : β < 1)
    (hλ : 0 < λ) (hc : 0 < c) (hk : 0 ≤ k)
    (hlow : ∀ᶠ X in atTop,
      c * X ^ (2 * λ * (β - σ) - λ * (1 - β) * (4 * σ * (1 - σ))) * (Real.log X) ^ (-k) ≤
        (ZeroDensity.zeroDensityCount σ (X ^ (λ * (1 - β))) : ℝ))
    (hgap : λ * (1 - β) * (4 * σ * (1 - σ)) <
      2 * λ * (β - σ) - λ * (1 - β) * (4 * σ * (1 - σ))) :
    False := by
  let hCarlson : CarlsonEventualMajorant σ :=
    Classical.choice (exists_carlsonEventualMajorant hσ hσ1)
  have hγpos : 0 < λ * (1 - β) := mul_pos hλ (sub_pos.mpr hβ1)
  have hγ0 : 0 ≤ λ * (1 - β) := le_of_lt hγpos
  have hq0 : 0 ≤ 4 * σ * (1 - σ) :=
    mul_nonneg (mul_nonneg (by norm_num : 0 ≤ (4 : ℝ)) hσ.le) (sub_nonneg.mpr hσ1.le)
  have hCarlsonX : ∀ᶠ X in atTop,
      (ZeroDensity.zeroDensityCount σ (X ^ (λ * (1 - β))) : ℝ) ≤
        (hCarlson.C : ℝ) * ‖(X ^ (λ * (1 - β))) ^ (4 * σ * (1 - σ)) *
          (Real.log (X ^ (λ * (1 - β)))) ^ 4‖ := by
    have htend : Filter.Tendsto (fun X : ℝ => X ^ (λ * (1 - β))) atTop atTop :=
      tendsto_rpow_atTop hγpos
    exact htend.eventually hCarlson.bound
  have hnorm : ∀ᶠ X in atTop,
      c * X ^ (2 * λ * (β - σ) - λ * (1 - β) * (4 * σ * (1 - σ))) * (Real.log X) ^ (-k) ≤
        (hCarlson.C : ℝ) * (λ * (1 - β)) ^ 4 *
          X ^ (λ * (1 - β) * (4 * σ * (1 - σ))) * (Real.log X) ^ 4 := by
    filter_upwards [hlow, hCarlsonX, Filter.eventually_ge_atTop (1 : ℝ)] with X hlowX hupX hX1
    have hXpos : 0 < X := lt_of_lt_of_le zero_lt_one hX1
    have hpow1 : (X ^ (λ * (1 - β))) ^ (4 * σ * (1 - σ)) =
        X ^ (λ * (1 - β) * (4 * σ * (1 - σ))) := by
      rw [Real.rpow_mul hXpos.le (λ * (1 - β)) (4 * σ * (1 - σ))]
    have hlog1 : Real.log (X ^ (λ * (1 - β))) = (λ * (1 - β)) * Real.log X :=
      Real.log_rpow hXpos (λ * (1 - β))
    have hlogX0 : 0 ≤ Real.log X := Real.log_nonneg hX1
    have hγlog0 : 0 ≤ (λ * (1 - β)) * Real.log X := mul_nonneg hγ0 hlogX0
    have hnorm4 : ‖(X ^ (λ * (1 - β))) ^ (4 * σ * (1 - σ)) *
          (Real.log (X ^ (λ * (1 - β)))) ^ 4‖ =
        X ^ (λ * (1 - β) * (4 * σ * (1 - σ))) * (λ * (1 - β)) ^ 4 * (Real.log X) ^ 4 := by
      rw [hpow1, hlog1, Real.norm_eq_abs]
      have hnonneg : 0 ≤ X ^ (λ * (1 - β) * (4 * σ * (1 - σ))) *
          ((λ * (1 - β)) * Real.log X) ^ 4 := by
        positivity
      rw [abs_of_nonneg hnonneg]
      have hγ4 : ((λ * (1 - β)) * Real.log X) ^ 4 = (λ * (1 - β)) ^ 4 * (Real.log X) ^ 4 := by
        rw [Real.mul_rpow hγ0 hlogX0]
      rw [hγ4]
      ring
    exact hlowX.trans (by simpa [hnorm4] using hupX)
  have hd : 0 < 2 * λ * (β - σ) - 2 * λ * (1 - β) * (4 * σ * (1 - σ)) := by
    linarith [hgap]
  have hfin : ∀ᶠ X in atTop,
      c * X ^ (2 * λ * (β - σ) - 2 * λ * (1 - β) * (4 * σ * (1 - σ))) * (Real.log X) ^ (-k) ≤
        ((hCarlson.C : ℝ) * (λ * (1 - β)) ^ 4) * (Real.log X) ^ 4 := by
    filter_upwards [hnorm, Filter.eventually_ge_atTop (1 : ℝ)] with X hnormX hX1
    have hXpos : 0 < X := lt_of_lt_of_le zero_lt_one hX1
    have hden : 0 < X ^ (λ * (1 - β) * (4 * σ * (1 - σ))) :=
      Real.rpow_pos_of_pos hXpos (λ * (1 - β) * (4 * σ * (1 - σ)))
    have hdiv : c * X ^ (2 * λ * (β - σ) - λ * (1 - β) * (4 * σ * (1 - σ))) *
          (Real.log X) ^ (-k) / X ^ (λ * (1 - β) * (4 * σ * (1 - σ))) ≤
        ((hCarlson.C : ℝ) * (λ * (1 - β)) ^ 4) *
          X ^ (λ * (1 - β) * (4 * σ * (1 - σ))) * (Real.log X) ^ 4 /
            X ^ (λ * (1 - β) * (4 * σ * (1 - σ))) := by
      exact div_le_div_of_nonneg_right hnormX hden.le
    have hdiv' : c * X ^ (2 * λ * (β - σ) - λ * (1 - β) * (4 * σ * (1 - σ))) *
          (Real.log X) ^ (-k) / X ^ (λ * (1 - β) * (4 * σ * (1 - σ))) =
        c * X ^ (2 * λ * (β - σ) - 2 * λ * (1 - β) * (4 * σ * (1 - σ))) * (Real.log X) ^ (-k) := by
      have hpow : X ^ (2 * λ * (β - σ) - λ * (1 - β) * (4 * σ * (1 - σ))) /
            X ^ (λ * (1 - β) * (4 * σ * (1 - σ))) =
          X ^ (2 * λ * (β - σ) - 2 * λ * (1 - β) * (4 * σ * (1 - σ))) := by
        calc
          X ^ (2 * λ * (β - σ) - λ * (1 - β) * (4 * σ * (1 - σ))) /
                X ^ (λ * (1 - β) * (4 * σ * (1 - σ)))
              = X ^ (2 * λ * (β - σ) - λ * (1 - β) * (4 * σ * (1 - σ))) *
                  X ^ (-(λ * (1 - β) * (4 * σ * (1 - σ)))) := by
            rw [div_eq_mul_inv, ← Real.rpow_neg]
          _ = X ^ (2 * λ * (β - σ) - λ * (1 - β) * (4 * σ * (1 - σ)) +
              -(λ * (1 - β) * (4 * σ * (1 - σ)))) := by
            rw [Real.rpow_add hXpos]
          _ = X ^ (2 * λ * (β - σ) - 2 * λ * (1 - β) * (4 * σ * (1 - σ))) := by
            congr 1
            ring
      calc
        c * X ^ (2 * λ * (β - σ) - λ * (1 - β) * (4 * σ * (1 - σ))) *
              (Real.log X) ^ (-k) / X ^ (λ * (1 - β) * (4 * σ * (1 - σ)))
            = c * (Real.log X) ^ (-k) *
              (X ^ (2 * λ * (β - σ) - λ * (1 - β) * (4 * σ * (1 - σ))) /
                X ^ (λ * (1 - β) * (4 * σ * (1 - σ)))) := by ring
        _ = c * (Real.log X) ^ (-k) *
              X ^ (2 * λ * (β - σ) - 2 * λ * (1 - β) * (4 * σ * (1 - σ))) := by
          rw [hpow]
        _ = c * X ^ (2 * λ * (β - σ) - 2 * λ * (1 - β) * (4 * σ * (1 - σ))) * (Real.log X) ^ (-k) := by
          ring
    have hdiv'' : ((hCarlson.C : ℝ) * (λ * (1 - β)) ^ 4) *
          X ^ (λ * (1 - β) * (4 * σ * (1 - σ))) * (Real.log X) ^ 4 /
            X ^ (λ * (1 - β) * (4 * σ * (1 - σ))) =
        ((hCarlson.C : ℝ) * (λ * (1 - β)) ^ 4) * (Real.log X) ^ 4 := by
      calc
        ((hCarlson.C : ℝ) * (λ * (1 - β)) ^ 4) *
              X ^ (λ * (1 - β) * (4 * σ * (1 - σ))) * (Real.log X) ^ 4 /
                X ^ (λ * (1 - β) * (4 * σ * (1 - σ)))
            = ((hCarlson.C : ℝ) * (λ * (1 - β)) ^ 4) * (Real.log X) ^ 4 *
              (X ^ (λ * (1 - β) * (4 * σ * (1 - σ))) /
                X ^ (λ * (1 - β) * (4 * σ * (1 - σ)))) := by ring
        _ = ((hCarlson.C : ℝ) * (λ * (1 - β)) ^ 4) * (Real.log X) ^ 4 * 1 := by
          rw [div_self hden.ne']
        _ = ((hCarlson.C : ℝ) * (λ * (1 - β)) ^ 4) * (Real.log X) ^ 4 := by ring
    rwa [hdiv', hdiv''] at hdiv
  have hB : 0 ≤ (hCarlson.C : ℝ) * (λ * (1 - β)) ^ 4 :=
    mul_nonneg (by exact_mod_cast hCarlson.C_nonneg)
      (Real.rpow_nonneg hγ0 4)
  exact powerGrowth_logGap_contradiction
    (A := c) (B := (hCarlson.C : ℝ) * (λ * (1 - β)) ^ 4) (k := k)
    (d := 2 * λ * (β - σ) - 2 * λ * (1 - β) * (4 * σ * (1 - σ)))
    hc hB hk hd hfin

/--
Terminal partial exclusion: if the coherent forcing supplies the lower
count for every seed real part `β > 14/17` and window exponent `λ > 0`,
then no non-trivial zero has real part `> 14/17`.

The exponent-gap arithmetic is unconditional: with `σ = 2/3` and
`λ = 1` the comparison `(1-β)·8/9 < 2(β-2/3) − (1-β)·8/9` is exactly
`β > 14/17`.
-/
theorem no_nontrivial_zero_re_gt_14_over_17_of_forcing
    (hforcing : ∀ β λ : ℝ, (14 / 17 : ℝ) < β → β < 1 → 0 < λ →
      ∃ c k : ℝ, 0 < c ∧ 0 < k ∧
        ∀ᶠ X in atTop,
          c * X ^ (2 * λ * (β - 2 / 3) - λ * (1 - β) * (8 / 9 : ℝ)) * (Real.log X) ^ (-k) ≤
            (ZeroDensity.zeroDensityCount (2 / 3 : ℝ) (X ^ (λ * (1 - β))) : ℝ)) :
  ...
  have hFalse := singleLayerForcing_carlson_contradiction
    (σ := 2 / 3) (β := ρ.re) (λ := 1) (c := c) (k := k)
    (hσ := by norm_num) (hσ1 := by norm_num)
    (hσβ := by linarith) (hβ1 := hβ1)
    (hλ := by norm_num) (hc := hc) (hk := le_of_lt hk) hlow hgap
