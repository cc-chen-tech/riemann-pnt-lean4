import PrimeNumberTheorem.ZeroDensityLayerBudgetActualCubicVerticalTailIntegralArithmetic
import PrimeNumberTheorem.ZeroDensityLayerBudgetAutomaticCubicContourIntegrability

namespace PrimeNumberTheorem

open Complex Set MeasureTheory
open scoped Interval
open ExplicitFormulaResidues

/-- The actual positive-height part of the dynamic cubic left edge is bounded
by the scalar `t^-3` tail.  In particular, no positive power of the outer
height survives the integration. -/
theorem exists_norm_integral_actualCubicPositiveVerticalTail_le :
    ∃ b C T0 : ℝ, 0 < b ∧ 0 ≤ C ∧ 4 ≤ T0 ∧
      ∀ x H : ℝ, 0 < x → T0 ≤ H →
        let a := dynamicCubicLeftBoundary b H
        ‖∫ t : ℝ in T0..H,
            thirdOrderExplicitFormulaIntegrand x ((a : ℂ) + t * I)‖ ≤
          x ^ a * C * (1 + Real.log (H + 6)) ^ 2 /
            (2 * T0 ^ 2) := by
  rcases exists_dynamicCubicLeftBoundary_high_logDeriv_le_log_sq with
    ⟨b, C, T0, hb, hC, hT0, hpointBase⟩
  refine ⟨b, C, T0, hb, hC, hT0, ?_⟩
  intro x H hx hH
  rcases hpointBase H hH with ⟨ha, _haHalf, hpoint⟩
  let a : ℝ := dynamicCubicLeftBoundary b H
  let L : ℝ := 1 + Real.log (H + 6)
  let K : ℝ := x ^ a * C * L ^ 2
  let f : ℝ → ℂ := fun t =>
    thirdOrderExplicitFormulaIntegrand x ((a : ℂ) + t * I)
  let g : ℝ → ℝ := fun t => K * t ^ (-3 : ℝ)
  have hTpos : 0 < T0 := (by norm_num : (0 : ℝ) < 4).trans_le hT0
  have hK : 0 ≤ K := by
    dsimp [K]
    positivity
  have hfInt : IntervalIntegrable f volume T0 H := by
    apply ContinuousOn.intervalIntegrable
    intro t htI
    rw [uIcc_of_le hH] at htI
    have htPos : 0 < t := hTpos.trans_le htI.1
    have htAbs : |t| = t := abs_of_pos htPos
    rcases hpoint t (by simpa [htAbs] using htI.1)
        (by simpa [htAbs] using htI.2) with ⟨hzeta, _hz1, _hlog⟩
    have hsEq : ((a : ℂ) + t * I) = (a : ℂ) + I * t := by ring
    have hzetaS : riemannZeta ((a : ℂ) + t * I) ≠ 0 := by
      rw [hsEq]
      exact hzeta
    have hs0 : ((a : ℂ) + t * I) ≠ 0 := by
      intro hs
      have him := congrArg Complex.im hs
      simp at him
      linarith
    have hs1 : ((a : ℂ) + t * I) ≠ 1 := by
      intro hs
      have him := congrArg Complex.im hs
      simp at him
      linarith
    have han := analyticAt_thirdOrderExplicitFormulaIntegrand_of_ne
      hx hs0 hs1 hzetaS
    have hpath : Continuous (fun u : ℝ => (a : ℂ) + u * I) := by fun_prop
    have hcomp : ContinuousAt
        (thirdOrderExplicitFormulaIntegrand x ∘
          fun u : ℝ => (a : ℂ) + u * I) t :=
      han.continuousAt.comp_of_eq hpath.continuousAt rfl
    simpa [f, Function.comp_def] using hcomp.continuousWithinAt
  have hzero : (0 : ℝ) ∉ [[T0, H]] := by
    rw [uIcc_of_le hH]
    simp only [mem_Icc, not_and_or]
    left
    linarith
  have hgInt : IntervalIntegrable g volume T0 H := by
    dsimp [g]
    exact (intervalIntegral.intervalIntegrable_rpow (μ := volume)
      (r := (-3 : ℝ)) (Or.inr hzero)).const_mul K
  have hmajor : ∀ᵐ t : ℝ, t ∈ Set.Ioc T0 H → ‖f t‖ ≤ g t := by
    filter_upwards [] with t htI
    have htPos : 0 < t := hTpos.trans_le htI.1.le
    have htAbs : |t| = t := abs_of_pos htPos
    rcases hpoint t (by simpa [htAbs] using htI.1.le)
        (by simpa [htAbs] using htI.2) with ⟨_hzeta, _hz1, hlog⟩
    let s : ℂ := (a : ℂ) + t * I
    have hsEq : s = (a : ℂ) + I * t := by dsimp [s]; ring
    have hlogS : ‖logDeriv riemannZeta s‖ ≤ C * L ^ 2 := by
      rw [hsEq]
      simpa [L] using hlog
    have hsNorm : t ≤ ‖s‖ := by
      calc
        t = |s.im| := by simp [s, abs_of_pos htPos]
        _ ≤ ‖s‖ := Complex.abs_im_le_norm s
    have hpow : t ^ 3 ≤ ‖s‖ ^ 3 := by
      exact pow_le_pow_left₀ htPos.le hsNorm 3
    have hnum : ‖logDeriv riemannZeta s‖ * x ^ a ≤ K := by
      dsimp [K, L]
      have hxpow : 0 ≤ x ^ a := Real.rpow_nonneg hx.le _
      nlinarith [hlogS]
    have hrpow : t ^ (-3 : ℝ) = 1 / t ^ 3 := by
      rw [Real.rpow_neg htPos.le]
      norm_num [Real.rpow_natCast]
    change ‖thirdOrderExplicitFormulaIntegrand x s‖ ≤ g t
    rw [norm_thirdOrderExplicitFormulaIntegrand_eq hx]
    have hsre : s.re = a := by simp [s]
    rw [hsre]
    calc
      ‖logDeriv riemannZeta s‖ * x ^ a / ‖s‖ ^ 3 ≤ K / ‖s‖ ^ 3 :=
        div_le_div_of_nonneg_right hnum (pow_nonneg (norm_nonneg _) _)
      _ ≤ K / t ^ 3 :=
        div_le_div_of_nonneg_left hK (pow_pos htPos 3) hpow
      _ = g t := by
        dsimp [g]
        rw [hrpow, div_eq_mul_inv, one_div]
  have hnorm := intervalIntegral.norm_integral_le_of_norm_le hH hmajor hgInt
  have hscalar := (integral_rpow_neg_three_le_half_inv_sq hTpos hH).2
  calc
    ‖∫ t : ℝ in T0..H, f t‖ ≤ ∫ t : ℝ in T0..H, g t := hnorm
    _ = K * (∫ t : ℝ in T0..H, t ^ (-3 : ℝ)) := by
      simp [g, intervalIntegral.integral_const_mul]
    _ ≤ K * (1 / (2 * T0 ^ 2)) :=
      mul_le_mul_of_nonneg_left hscalar hK
    _ = x ^ a * C * (1 + Real.log (H + 6)) ^ 2 / (2 * T0 ^ 2) := by
      simp [K, L, div_eq_mul_inv]

end PrimeNumberTheorem
