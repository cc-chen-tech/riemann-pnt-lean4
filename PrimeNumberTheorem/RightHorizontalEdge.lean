...
/-- The first-order explicit-formula integrand is genuinely integrable on a
compact horizontal segment contained in `Re(s) > 1`. -/
theorem intervalIntegrable_explicitFormulaIntegrand_horizontal_right
    {x ε c T : ℝ} (hx : 1 ≤ x) (hε : 0 < ε) (hc : 1 + ε ≤ c) :
    IntervalIntegrable
      (fun σ : ℝ => explicitFormulaIntegrand x ((σ : ℂ) + T * I))
      volume (1 + ε) c := by
  apply ContinuousOn.intervalIntegrable
  intro σ hσ
  rw [Set.uIcc_of_le hc] at hσ
  rcases hσ with ⟨hσlow, hσhigh⟩
  have hre : 1 < (((σ : ℂ) + T * I).re) := by
    simp only [Complex.add_re, ofReal_re, mul_re, ofReal_im, I_re, I_im,
      zero_mul, mul_zero, sub_zero, add_zero]
    linarith
  have hs0 : (σ : ℂ) + T * I ≠ 0 := by
    ...
    simp only [Complex.add_re, ofReal_re, mul_re, ofReal_im, I_re, I_im,
      zero_mul, mul_zero, sub_zero, add_zero, zero_re] at h
    linarith
  have hs1 : (σ : ℂ) + T * I ≠ 1 := by
    intro hs
    have h := congrArg Complex.re hs
    simp only [Complex.add_re, ofReal_re, mul_re, ofReal_im, I_re, I_im,
      zero_mul, mul_zero, sub_zero, add_zero, one_re] at h
    linarith
  have hzeta : riemannZeta ((σ : ℂ) + T * I) ≠ 0 :=
    riemannZeta_ne_zero_of_one_le_re hre.le
  have han : ContinuousAt (explicitFormulaIntegrand x) ((σ : ℂ) + T * I) :=
    (analyticAt_explicitFormulaIntegrand_of_ne_zero_of_ne_one_of_zeta_ne_zero
      (zero_lt_one.trans_le hx) hs0 hs1 hzeta).continuousAt
  have hmap : ContinuousAt (fun r : ℝ => ((r : ℂ) + T * I)) σ := by
    fun_prop
  change ContinuousWithinAt
    (explicitFormulaIntegrand x ∘ fun r : ℝ => ((r : ℂ) + T * I)) _ σ
  exact (ContinuousAt.comp
    (f := fun r : ℝ => ((r : ℂ) + T * I))
    (x := σ) (g := explicitFormulaIntegrand x) han hmap).continuousWithinAt
    ...
    IntervalIntegrable
      ...
      ‖∫ σ : ℝ in (1 + ε)..c,
        ...
        (vonMangoldtLSeriesNorm ε * x ^ c / T) *
          (c - (1 + ε)) := by
  refine ⟨intervalIntegrable_explicitFormulaIntegrand_horizontal_right hx hε hc, ?_⟩
  have hbound := intervalIntegral.norm_integral_le_of_norm_le_const
    ...
    IntervalIntegrable
      ...
      ‖∫ σ : ℝ in (1 + ε)..c,
        ...
        (vonMangoldtLSeriesNorm ε * x ^ c / T) *
          (c - (1 + ε)) := by
  have hintegrable :=
    intervalIntegrable_explicitFormulaIntegrand_horizontal_right
      (T := -T) hx hε hc
  refine ⟨by simpa [sub_eq_add_neg] using hintegrable, ?_⟩
  have hbound := intervalIntegral.norm_integral_le_of_norm_le_const
    (f := fun σ : ℝ => explicitFormulaIntegrand x ((σ : ℂ) - T * I))
    (a := 1 + ε) (b := c)
    (C := vonMangoldtLSeriesNorm ε * x ^ c / T)
  ...
  calc
    ...
    _ ≤ (vonMangoldtLSeriesNorm ε * x ^ c / T) *
        ...
        (vonMangoldtLSeriesNorm ε * x ^ c / T) *
          (c - (1 + ε)) :=
      add_le_add
        (norm_horizontal_right_firstOrderContour_neg_height_le hx hε hc hT).2
        (norm_horizontal_right_firstOrderContour_le hx hε hc hT).2
    _ = _ := by ring
