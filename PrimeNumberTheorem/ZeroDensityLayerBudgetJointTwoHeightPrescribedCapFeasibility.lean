...
/-!
# Prescribed-cap joint two-height feasibility

This file strengthens the joint parameter arithmetic by forcing a prescribed
real-part ceiling `theta` below the selected Carlson endpoint `tau`.
-/

namespace PrimeNumberTheorem

/-- Fixed endpoints satisfying the low-layer and Carlson contour-floor
conditions admit one common outer height, both balanced cuts, and positive
strict margins. -/
theorem exists_jointTwoHeightTargetAmplitudeMargins_of_endpoints
    {beta sigma tau : ℝ}
    (hbetaOne : beta < 1)
    (hsigmaHalf : 1 / 2 < sigma) (hsigmaOne : sigma < 1)
    (hsigmaThreshold : sigma < (3 * beta - 1) / 2)
    (hhighFloor :
      targetAmplitudeCarlsonTwoHeightBalancedSlope sigma *
          (1 - beta) + tau - beta < 0) :
    ∃ alpha gammaLow gammaHigh epsilonLow epsilonHigh : ℝ,
      1 - beta < alpha ∧
      0 < alpha ∧
      alpha ≤ 1 ∧
      gammaLow = alpha / 2 ∧
  ...
  have hfloorHighUpper : 1 - beta < highUpper := by
    ...
    have hhighFloor' :
        slope * (1 - beta) + tau - beta < 0 := by
      simpa [slope] using hhighFloor
    nlinarith
  ...
  have hcarlsonHigh :
    ...
    linarith
  exact
    ⟨alpha, gammaLow, gammaHigh, epsilonLow, epsilonHigh,
...
/-- A prescribed global real-part ceiling below the canonical `2 / 3`
threshold line can be placed strictly below the selected Carlson endpoint
without losing any joint two-height margin. -/
theorem exists_jointTwoHeightTargetAmplitudeParameters_above_cap
    {beta theta : ℝ}
  ...
  have hthresholdTauUpper : threshold < tauUpper := by
    ...
    dsimp [tauUpper]
    linarith
  have hsigmaTauUpper : sigma < tauUpper :=
    hsigmaThreshold.trans hthresholdTauUpper
  have hthetaThreshold : theta < threshold := by
    simpa [threshold] using htheta
  have hthetaTauUpper : theta < tauUpper :=
    hthetaThreshold.trans hthresholdTauUpper
  let lower : ℝ := max sigma theta
  have hlowerTauUpper : lower < tauUpper := by
    dsimp [lower]
    exact max_lt hsigmaTauUpper hthetaTauUpper
  let tau : ℝ := (lower + tauUpper) / 2
  have hlowerTau : lower < tau := by
    dsimp [tau]
    linarith
  ...
  have hsigmaTau : sigma < tau :=
    (le_max_left sigma theta).trans_lt hlowerTau
  have hthetaTau : theta < tau :=
    (le_max_right sigma theta).trans_lt hlowerTau
  ...
  have hhighFloor :
    ...
    dsimp [tauUpper, slope] at htauTauUpper ⊢
    linarith
  have hsigmaThreshold' :
      sigma < (3 * beta - 1) / 2 := by
    simpa [threshold] using hsigmaThreshold
  rcases
      exists_jointTwoHeightTargetAmplitudeMargins_of_endpoints
        hbetaOne hsigmaHalf hsigmaOne
        hsigmaThreshold' hhighFloor with
    ⟨alpha, gammaLow, gammaHigh, epsilonLow, epsilonHigh,
