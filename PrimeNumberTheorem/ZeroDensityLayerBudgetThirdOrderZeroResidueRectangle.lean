...
theorem exists_boundaryRectIntegral_thirdOrderExplicitFormulaIntegrand_eq_residue_sum_explicit_zeroPole
  ...
  have hKnhds : K ∈ 𝓝 (0 : ℂ) := by
    rw [mem_nhds_iff]
    refine ⟨Ioo a c ×ℂ Ioo (-W) W, ?_, isOpen_Ioo.reProdIm isOpen_Ioo, ?_⟩
    · intro z hz
      change z.re ∈ Ioo a c ∧ z.im ∈ Ioo (-W) W at hz
      change z.re ∈ uIcc a c ∧ z.im ∈ uIcc (-W) W
      rw [uIcc_of_le hac, uIcc_of_le hWW]
      exact ⟨Ioo_subset_Icc_self hz.1, Ioo_subset_Icc_self hz.2⟩
    · change (0 : ℝ) ∈ Ioo a c ∧ (0 : ℝ) ∈ Ioo (-W) W
      constructor <;> constructor <;> linarith
  obtain ⟨poles, residue, quadratic, cubic, G, h0mem, hpolesK,
      hpolesType, hresidue, hresidue0, hcubic, hG, heq⟩ :=
    exists_thirdOrderExplicitFormula_zeroPole_regularization_explicit_zero_residue hx hK hKnhds
  ...
  refine ⟨poles, residue, cubic, h0mem, hpolesInterior,
    hpolesType, hresidue, hresidue0, hcubic, ?_⟩
