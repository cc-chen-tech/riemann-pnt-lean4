...
/-- The measurable moving-prefix Gaussian integrand is globally integrable
under the same scale separation used by the maximal moment theorem. -/
theorem integrable_gaussian_normSq_dyadicMovingPrefixMollifiedPolynomial
    ...
    Integrable fun t : ℝ =>
      Real.exp (-((t - w) ^ 2) / Delta ^ 2) *
        Complex.normSq
          (dyadicMovingPrefixMollifiedPolynomial K (cutoff t) X t) := by
