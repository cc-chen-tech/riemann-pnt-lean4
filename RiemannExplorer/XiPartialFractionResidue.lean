...
/-- **留数定理（ξ'/ξ 侧，一般零点）**：在每个 `ξ` 零点 `s₀` 处，
`(s - s₀)·ξ'(s)/ξ(s)` 的去心极限等于 `ξ` 在 `s₀`
的解析重数（`analyticOrderNatAt`，≥ 1）。

证明：由恒等定理（`ξ 0 = 1/2 ≠ 0`），`ξ` 在 `s₀` 不局部恒零，
故解析阶为某个自然数 `n`；`ξ s₀ = 0` 给出 `n ≥ 1`。局部因子分解
`ξ z = (z - s₀)ⁿ · g z`（`g` 解析、`g s₀ ≠ 0`，
`AnalyticAt.analyticOrderAt_eq_natCast`）逐项求导后在去心邻域上
`(s - s₀)·ξ'/ξ = (n·g + (s - s₀)·g')/g`，极限为 `n·g s₀/g s₀ = n`。 -/
theorem tendsto_sub_mul_logDeriv_xiFunction_of_zero {s₀ : ℂ}
    (hξ0 : xiFunction s₀ = 0) :
    Filter.Tendsto (fun s => (s - s₀) * (deriv xiFunction s / xiFunction s))
      (𝓝[≠] s₀) (𝓝 ((analyticOrderNatAt xiFunction s₀ : ℂ))) := by
  classical
  have hξa : AnalyticAt ℂ xiFunction s₀ := differentiable_xiFunction.analyticAt _
  -- 恒等定理：ξ 不在 ρ₀ 附近恒为零（否则 ξ ≡ 0，与 ξ 0 = 1/2 矛盾）
  have hne : ¬ ∀ᶠ z in 𝓝 s₀, xiFunction z = 0 := by
    intro h
    ...
    norm_num at h00
  have horder_top : analyticOrderAt xiFunction s₀ ≠ ⊤ :=
    fun h => hne (analyticOrderAt_eq_top.mp h)
  set n := analyticOrderNatAt xiFunction s₀ with hn
  have hn_eq : analyticOrderAt xiFunction s₀ = (n : ℕ∞) := by
    rw [hn]
  ...
  have hn_pos : 0 < n := by
    ...
    · exact hpos
  have hg_eq' : ∀ᶠ z in 𝓝 s₀, xiFunction z = (z - s₀) ^ n * g z := by
  ...
  have hgder_cont : ContinuousAt (deriv g) s₀ := by
    obtain ⟨U, hU_nhds, hU⟩ := hg_an.eventually_analyticAt.exists_mem
    exact (AnalyticOnNhd.deriv (fun z hz => hU z hz)) s₀
      (mem_of_mem_nhds hU_nhds) |>.continuousAt
  have hsub : Filter.Tendsto (fun s : ℂ => s - s₀) (𝓝 s₀) (𝓝 0) := by
    have h := (continuousAt_id.sub continuousAt_const :
      ContinuousAt (fun s : ℂ => s - s₀) s₀).tendsto
    simpa using h
  -- 连续模型 q(s) = (n·g s + (s - ρ₀)·g' s)/g s 的极限为 n
  have hnum : Filter.Tendsto (fun s => (↑n : ℂ) * g s + (s - s₀) * deriv g s)
      (𝓝 s₀) (𝓝 ((↑n : ℂ) * g s₀ + 0 * deriv g s₀)) :=
    (tendsto_const_nhds.mul hg_cont.tendsto).add (hsub.mul hgder_cont.tendsto)
  have hT : Filter.Tendsto
      (fun s => ((↑n : ℂ) * g s + (s - s₀) * deriv g s) / g s)
      (𝓝 s₀) (𝓝 (((↑n : ℂ) * g s₀ + 0 * deriv g s₀) / g s₀)) :=
    hnum.div hg_cont.tendsto hg_ne
  have hlim : ((↑n : ℂ) * g s₀ + 0 * deriv g s₀) / g s₀ = (↑n : ℂ) := by
  ...
  have hmodel : Filter.Tendsto
    ...
    exact hT.mono_left (nhdsWithin_le_nhds (s := {s₀}ᶜ))
  -- 去心邻域上的逐点恒等式
  have heq : (fun s => (s - s₀) * (deriv xiFunction s / xiFunction s))
      =ᶠ[𝓝[≠] s₀]
      (fun s => ((↑n : ℂ) * g s + (s - s₀) * deriv g s) / g s) := by
    ...
    show (s - s₀) * (deriv xiFunction s / xiFunction s) =
      ((↑n : ℂ) * g s + (s - s₀) * deriv g s) / g s
    have hder_pow : deriv (fun z : ℂ => (z - s₀) ^ n) s =
        ↑n * (s - s₀) ^ (n - 1) := by
      have h1 : deriv (fun z : ℂ => (z - s₀) ^ n) s =
          ↑n * (s - s₀) ^ (n - 1) * deriv (fun z : ℂ => z - s₀) s :=
        deriv_pow (differentiableAt_id.sub (differentiableAt_const s₀)) n
      have h2 : deriv (fun z : ℂ => z - s₀) s = 1 := by
        rw [deriv_sub_const]
        exact (hasDerivAt_id' s).deriv
      rw [h1, h2, mul_one]
    have hder_prod : deriv (fun z : ℂ => (z - s₀) ^ n * g z) s =
        ↑n * (s - s₀) ^ (n - 1) * g s + (s - s₀) ^ n * deriv g s := by
      have hm : deriv (fun z : ℂ => (z - s₀) ^ n * g z) s =
          deriv (fun z : ℂ => (z - s₀) ^ n) s * g s +
            (s - s₀) ^ n * deriv g s :=
        deriv_mul ((differentiableAt_id.sub (differentiableAt_const s₀)).pow n)
          hg_an_s.differentiableAt
      rw [hm, hder_pow]
    have hpow : (s - s₀) ^ n = (s - s₀) ^ (n - 1) * (s - s₀) := by
...
/-- **留数定理（ξ'/ξ 侧）**：上半平面零点版本，
`tendsto_sub_mul_logDeriv_xiFunction_of_zero` 的特化。 -/
theorem tendsto_sub_mul_logDeriv_xiFunction (ρ₀ : UpperHalfPlaneNontrivialZero) :
    Filter.Tendsto (fun s => (s - (ρ₀ : ℂ)) * (deriv xiFunction s / xiFunction s))
      (𝓝[≠] (ρ₀ : ℂ)) (𝓝 ((analyticOrderNatAt xiFunction (ρ₀ : ℂ) : ℂ))) :=
  tendsto_sub_mul_logDeriv_xiFunction_of_zero
