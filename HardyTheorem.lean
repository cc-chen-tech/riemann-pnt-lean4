...
lemma weightedIntegralOf_eventually_positive_of_tail_dominates
  ...
  have hg : Continuous g := by
    have hw : Continuous fun t : ℝ => weightFunction n t := by
      simpa [weightFunction] using (by fun_prop : Continuous fun t : ℝ => t ^ (2 * n))
    exact hw.mul hf
  have htail_gt :
...
threshold. -/
lemma approximate_functional_equation_target_of_eventually_and_bounded_patch
