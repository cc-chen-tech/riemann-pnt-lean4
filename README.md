...
| `ZeroFreeRegion.logDerivVerticalLogBound_of_affine_log_norm_add_three_bound_high_height` / derivative-lower-bound, signed, and real-part variants | `lemma` | Packages the full-height normalizers directly into the named `LogDerivVerticalLogBound`, `NegLogDerivVerticalLogBound`, and `ReNegDerivDivVerticalLogBound` interfaces. | Lets a future zeta-specific `A + B log(‖σ+it‖+3)` estimate, or the derivative/lower-bound pair above, plug into the exact mid-term vertical-bound target without unpacking the existential normalizer by hand. |
...
`x^s/s` is bounded by `(1/2) * x^Re(s)`, the finite retained trivial-zero
contribution sum is bounded by the corresponding finite sum of those
amplitudes, and the truncation cardinality is bounded by `Nat.floor (T / 2)`.
This still does not prove the infinite trivial-zero correction term.
