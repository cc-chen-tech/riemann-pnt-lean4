...
抵消，因此不能把二者分别压到对数小量来闭合 `hexcessive`。当前主线
已经转向保留 Hermitian 相位的短窗口核估计：先分驻相与非驻相区域，
...
`|frequencyGap| < H/T` 的支持点对只能是对角项。尚缺的是给
`sum |c_q|^2 / localFrequencySeparation(q)` 足够尖锐的实际算术上界，
并将该界与对角能量、shift 积分和参数选择合成，最终验证
`hexcessive` 所需的统一总预算。
