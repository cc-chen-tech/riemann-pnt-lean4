...
| 分支或 PR | 当前进展 | 尚未闭合的边界 |
|---|---|---|
| [`research/hardy-littlewood`](https://github.com/cc-chen-tech/riemann-pnt-lean4/tree/research/hardy-littlewood) | 已证明 `hardy_littlewood_lower_bound_target_proved` 和奇重数强化版；Selberg 工作现已把 `hsmall` 与 `hexcessive` 统一到同一个 `zeta^(-1/2)` mollifier。已证明 sqrt-zeta 短区间绝对质量下界及其 gap-sum 坏窗口归约、临界线上 mollifier 的 `2 sqrt X` 范数界、实际 signed mollified Hardy 函数到有限 `thetaModel` 的统一点态近似、该模型与有限三重相位多项式的精确等式、lag 自相关的局部统一误差控制、lag 区域始终留在同一 dyadic 高度带的几何引理，以及公共 Hardy 相位差乘任意实频率多项式的 frequency-gap 界。重复频率已归入同频主块，不再错误地假设不同三重索引具有不同频率。`hsmall` 的高区间还被精确降维为受限除数对的 lcm/harmonic 公式和 totient 加权有限平方和 | Selberg `T log T` 下界仍是目标。剩余硬问题是：(1) 对三重相位多项式的同频主块和真正非对角 gap sum 给出尖锐算术上界；(2) 将点态近似、dyadic lag 几何和模型自相关估计拼成实际 sqrt-zeta mollified Hardy 自相关的 `T * eta^2 / 24` 预算；(3) 保留 `min(H, 2/log k)` 衰减与实际系数抵消，闭合 `hsmall` 的高区间；完成后再统一选择 `X(T)`、`eta(T)` 和窗口常数 |
