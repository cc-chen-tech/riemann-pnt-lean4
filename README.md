...
| 分支或 PR | 当前进展 | 尚未闭合的边界 |
|---|---|---|
| [`research/hardy-littlewood`](https://github.com/cc-chen-tech/riemann-pnt-lean4/tree/research/hardy-littlewood) | 已证明 `hardy_littlewood_lower_bound_target_proved` 和奇重数强化版；Selberg 工作已完成 lag/Fourier/Hardy 相位模块，构造经典 `zeta^(-1/2)` 系数，并证明低区间恒等式、有限 zeta 截断的 collected coefficient 桥，以及实际有限系数的统一 `(15/4)H^2` 滑动能量界 | Selberg `T log T` 下界仍是目标；还需控制高区间能量、off-diagonal gap sum、一致均值误差，并把 sqrt-zeta mollifier 接到坏窗口测度层 |
