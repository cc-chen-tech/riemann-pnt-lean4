...
| 分支或 PR | 当前进展 | 尚未闭合的边界 |
|---|---|---|
| [`research/hardy-littlewood`](https://github.com/cc-chen-tech/riemann-pnt-lean4/tree/research/hardy-littlewood) | 已证明 `hardy_littlewood_lower_bound_target_proved` 和奇重数强化版；Selberg 工作现已把 `hsmall` 与 `hexcessive` 统一到同一个 `zeta^(-1/2)` mollifier。已证明 sqrt-zeta 短区间绝对质量下界及其 gap-sum 坏窗口归约、临界线上 mollifier 的 `2 sqrt X` 范数界、通用 signed sliding second-moment 与 lag 恒等式、sqrt-zeta 好窗口推出 Hardy `Z` 局部变号的最终拼装桥，以及从实际 sqrt-zeta lag 自相关预算推出 `hexcessive <= T/24`。`hsmall` 的高区间还被精确降维为受限除数对的 lcm/harmonic 公式和 totient 加权有限平方和；完整 gap sum 已拆成 diagonal 与 off-diagonal 两项 | Selberg `T log T` 下界仍是目标。剩余硬问题是：(1) 保留 `min(H, 2/log k)` 衰减和实际系数抵消，给出高区间与 off-diagonal 的尖锐上界；(2) 对实际 sqrt-zeta mollified Hardy 自相关证明 `T * eta^2 / 24` 的 lag 预算；完成后再统一选择 `X(T)`、`eta(T)` 和窗口常数 |
