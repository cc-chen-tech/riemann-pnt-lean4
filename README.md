...
| `ZeroFreeRegion.norm_logDeriv_le_four_mul_max_add_log_three_div_of_sphere_log_norm_le` / `ZeroFreeRegion.exists_good_radius_log_norm_and_logDeriv_riemannZeta_factor_le_jensen_bound` | `lemma` | Propagates a circle bound by the maximum-modulus principle, uses the proved center margin `‖g(c)‖≥1/3`, and obtains an explicit `‖g'/g‖` bound at the center `2+it` for the Jensen-selected zero-removed zeta factor, with denominator replaced by the fixed radius floor `a`. | The Borel/Cauchy center conversion for this selected factor is proved.  It is not yet the zero-candidate regular-part estimate at `sigma+it`; zeta vertical growth, transfer to the required center, and sharp control of the good-circle separation loss remain open. |
