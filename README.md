...
| `PrimeNumberTheorem.RiemannVonMangoldt.riemannZeroCount_sub_eq_gammaPhase_add_zetaHalfPathArgument` / `exists_abs_zetaHalfPathArgument_le_log` | `theorem` | Identifies the multiplicity-weighted zero-count increment exactly with a Gamma phase increment plus a zeta-only half-boundary phase, then bounds the latter by `O(log U + log T)` at good heights. | Closes the argument-principle-to-phase bridge; this is an exact proved identity, not a route predicate. |
| `PrimeNumberTheorem.RiemannVonMangoldt.exists_abs_riemannZeroCount_sub_mainTerm_le_log_of_goodHeight` | `theorem` | Combines the exact phase identity, the vertical Gamma asymptotic, and the zeta boundary estimate to prove the standard two-term zero-counting formula at every sufficiently high good height. | Establishes the Riemann-von Mangoldt formula before removing the boundary-avoidance condition. |
| `PrimeNumberTheorem.RiemannVonMangoldt.exists_abs_riemannZeroCount_sub_mainTerm_le_log` | `theorem` | Chooses good heights immediately above and below any `T >= 8`, uses monotonicity of the multiplicity-weighted count, and controls the explicit main term over unit intervals by the mean value theorem. | Proves for all `T >= 8` that `|N(T) - (T/(2*pi))*log(T/(2*pi)) + T/(2*pi)| <= C*(1+log(T+6))`; this is the full classical Riemann-von Mangoldt asymptotic, not RH. |
...
bound.  This theorem is also specialized directly to the collected
nonconstant Selberg polynomial, so the remaining input is now exactly its
per-block coefficient energy; the specialization does not prove the Selberg
target by itself.
