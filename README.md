...
| `ZeroFreeRegion.analyticOrderNatAt_riemannZeta_pos_of_zero` / `ZeroFreeRegion.exists_punctured_ball_norm_logDeriv_riemannZeta_sub_analyticOrderNatAt_mul_inv_le_of_zero_auto` / signed and closed-ball variants | `lemma` | For any actual zero `ρ ≠ 1`, proves the natural analytic multiplicity `analyticOrderNatAt riemannZeta ρ` is positive and automatically obtains a local punctured-ball bound for `logDeriv ζ - n/(z-ρ)` with `n` chosen as that multiplicity. | Removes a manual `analyticOrderAt ζ ρ = n` hypothesis from the zero-local principal-part API; this is a genuine local regular-part theorem, but it still supplies only a local constant, not the missing uniform `O(log |t|)` coefficient. |
