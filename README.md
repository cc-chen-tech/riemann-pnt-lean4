...
multiplicity divided by `R-d`.  Connecting zeta's integer-valued divisor to
these natural multiplicities and completing the inner/outer partition remain
under construction; no uniform regular-part theorem is claimed.
...
| `ZeroFreeRegion.translatedCanonicalNumerator` / `canonicalNumeratorProduct` and their boundary, analytic, nonvanishing, log-derivative, and norm-bound lemmas | `def` + `lemma` | Replaces a finite family of ordinary zero factors by disk-canonical numerators, preserving the full boundary norm and bounding the correction by total multiplicity divided by `R-d`. | Removes the old `log(cardinality)` separation loss at the finite-product level. Connecting the zeta divisor and completing the final regular-part estimate remain open. |
