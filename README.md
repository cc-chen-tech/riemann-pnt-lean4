...
| `ZeroFreeRegion.exists_logDeriv_affine_log_norm_add_three_bound_on_verticalRegion_of_compact_band_and_high_height` / signed `...negLogDeriv...` variants and `...VerticalLogBound...` wrappers | `lemma` | Patches the compact bounded-height `±logDeriv ζ` norm bounds with future high-height affine logarithmic estimates on `‖z‖+3`, then feeds them into `LogDerivVerticalLogBound` / `NegLogDerivVerticalLogBound`. | Converts the remaining signed or unsigned vertical log-derivative task into one high-height zeta-specific estimate plus an already-proved compact patch; it does not prove that high-height estimate. |
