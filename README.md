...
| `ZeroFreeRegion.exists_logDeriv_affine_log_norm_add_three_bound_on_verticalRegion_of_compact_band_and_high_height` / `logDerivVerticalLogBound_of_compact_band_and_high_height_affine_log_norm_add_three_bound` | `lemma` | Patches the compact bounded-height `logDeriv ζ` norm bound with a future high-height affine logarithmic estimate on `‖z‖+3`, then feeds it into `LogDerivVerticalLogBound`. | Converts the remaining vertical log-derivative task into one high-height zeta-specific estimate plus an already-proved compact patch; it does not prove that high-height estimate. |
