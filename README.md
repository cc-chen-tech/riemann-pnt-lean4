...
| `ZeroFreeRegion.exists_logDeriv_regular_part_sigmaOf_log_bound_of_absolute_convergence` | `lemma` | For fixed `a>0`, proves `‖logDeriv ζ(1+a/log|t|+it) - (1+a/log|t|-β)⁻¹‖ ≤ B(a) log |t|` whenever `|t|≥3` and `β<1`. | Provides a real theorem on the regular-part side using only absolute convergence plus the principal-part estimate `1/(σ-β)≤(log|t|)/a`; the `B(a)` dependence is exactly why the uniform Borel/Jensen regular-part bound remains open. |
