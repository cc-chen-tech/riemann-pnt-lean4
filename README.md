...
positive and signed `logDeriv ζ` half-radius bridges whose real-part and center
hypotheses are already stated in the `log(|t|+3)` height scale.  These handoff
lemmas do not prove the missing polynomial growth or boundary-strip
logarithmic-derivative estimate itself.  On the pole side, the local decomposition
