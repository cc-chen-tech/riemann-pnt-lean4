...
| `ZeroFreeRegion.sigmaOf_log_borel_radius_affine_majorant_pos` and the `..._borel_radius_of_pos_A` bridges | `lemma` | Discharge the Borel-Carathéodory positivity side-condition `M > 0` from the natural hypotheses `0 < Are` and `0 ≤ Bre` for the canonical moving disk. | Removes a nuisance hypothesis from the local regular-part handoff; the actual affine real-part bound on the disk remains the hard analytic input. |
