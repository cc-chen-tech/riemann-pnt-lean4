...
| `ZeroFreeRegion.finsum_divisor_riemannZeta_closedBall_eq_finsum_mem_of_le` / `ZeroFreeRegion.exists_good_radius_log_norm_riemannZeta_factor_le_jensen_bound` | `lemma` | Identifies divisor mass across nested disks, applies Jensen to eliminate the unknown total multiplicity, and bounds `log‖g‖` on a positive-radius good circle using only inner/outer zeta growth data and explicit geometry. | Produces the concrete analytic-factor boundary input for a later Borel/Cauchy estimate; the actual zeta vertical-growth theorem and the final uniform `g'/g = O(log |t|)` estimate remain open. |
