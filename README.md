...
| `ZeroFreeRegion.half_le_norm_riemannZeta_of_three_le_re` / `ZeroFreeRegion.norm_logDeriv_riemannZeta_le_two_mul_re_zeta_two_of_three_le_re` | `lemma` | Proves `‖ζ(s)‖ ≥ 1/2` and then `‖logDeriv ζ(s)‖ ≤ 2 Re(ζ(2))` on `3 ≤ Re(s)`. | Far-right nonvanishing-with-margin and logarithmic-derivative control, useful as a checked boundary condition for future strip arguments. |
