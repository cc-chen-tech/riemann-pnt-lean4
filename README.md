...
| `ZeroFreeRegion.analyticAt_logDeriv_riemannZeta_closedBall_sigmaOf_log_add_I_mul_of_radius_le_width_of_height_add_le` | `lemma` | Proves `logDeriv ζ` is analytic at every point of a closed disk centered at `σ+it`, for `σ = 1 + a / log |t|`, when the radius stays inside `Re(s) ≥ 1` and the height condition keeps the disk away from the pole. | Provides the local disk regularity input needed before applying Borel-Carathéodory/Jensen estimates around moving high-height centers; it still does not prove the missing high-height logarithmic bound. |
