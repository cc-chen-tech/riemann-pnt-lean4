...
| `ZeroFreeRegion.eventually_ne_zero_riemannZeta_nhdsNE_of_ne_one` / `exists_punctured_ball_riemannZeta_ne_zero_of_ne_one` / coordinate form | `lemma` | Proves local isolation of zeta zeros away from the pole, both as a punctured-filter statement and as punctured-ball/horizontal-coordinate radius bounds. | Supplies denominator safety around actual zeros for local principal-part, Jensen, and zero-counting arguments. |
