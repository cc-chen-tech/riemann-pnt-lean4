...
Prime Number Theorem through the classical de la Vallee Poussin route. It does
not prove the Riemann Hypothesis or a classical effective PNT error term.
...
four chain-gap buckets. The ordinary PNT is theorem-level; RH, Hardy,
Vinogradov-Korobov, and effective PNT error terms remain outside the proved
boundary.
...
classical c/log zero-free region, and the resulting ordinary PNT through a
multiplicity-aware moving-height explicit formula.
```

Do not claim:

- first formalization of PNT;
- proof of an effective PNT error term;
...
moving-height explicit-formula machinery
```

not as:

```text
proof of RH or an effective PNT error term
```
...
| `PrimeNumberTheorem.lean` and `PrimeNumberTheorem/PNTFromDynamicPerron.lean` | 0 | Ordinary PNT and explicit formula proved; RH-scale error predicates remain open because RH is open |
