...
/-- Uniform weak moving-line regular-part estimate on margins bounded below.

If the moving-line parameter satisfies `a0 <= a`, the absolute-convergence
argument gives a single constant depending on `a0`, not on the varying `a`.
The theorem still does not handle the final zero-free-region limit `a -> 0`. -/
lemma exists_logDeriv_regular_part_sigmaOf_log_bound_of_absolute_convergence_uniform_of_le
    {a0 : ℝ} (ha0 : 0 < a0) :
  ...
  rcases exists_norm_logDeriv_riemannZeta_sigma_ge_sigmaOf_log_shift_pair_le_log_abs
      ha0 with ⟨B₀, hB₀, hbound⟩
  refine ⟨B₀ + 1 / a0, add_nonneg hB₀ (by positivity), ?_⟩
  intro a β t ha0a ht hβ
  let σ : ℝ := 1 + a / Real.log |t|
  have ht2 : 2 ≤ |t| := by linarith
  have hlog_pos : 0 < Real.log |t| := log_abs_pos_of_two_le ht2
  have hlog_nonneg : 0 ≤ Real.log |t| := hlog_pos.le
  have ha_pos : 0 < a := lt_of_lt_of_le ha0 ha0a
  have hσ_lower : 1 + a0 / Real.log |t| ≤ σ := by
    have hdiv : a0 / Real.log |t| ≤ a / Real.log |t| :=
      div_le_div_of_nonneg_right ha0a hlog_nonneg
    change 1 + a0 / Real.log |t| ≤ 1 + a / Real.log |t|
    linarith
  have hlogDeriv :
      ‖logDeriv riemannZeta ((σ : ℂ) + I * t)‖ ≤
        B₀ * Real.log |t| := by
  ...
  have hgap_pos : 0 < σ - β := by
    have hσ_gt : 1 < σ := by
      simpa [σ] using sigmaOf_log_gt_one (T0 := 2) (a := a) (t := t)
        (by norm_num) ha_pos ht2
    linarith
  have hgap_ge : a0 / Real.log |t| ≤ σ - β := by
    have hdiv : a0 / Real.log |t| ≤ a / Real.log |t| :=
      div_le_div_of_nonneg_right ha0a hlog_nonneg
    have ha_gap : a / Real.log |t| ≤ σ - β := by
      change a / Real.log |t| ≤ (1 + a / Real.log |t|) - β
      linarith
    exact hdiv.trans ha_gap
  have hprincipal :
      ‖(((σ - β : ℝ) : ℂ)⁻¹)‖ ≤ (1 / a0) * Real.log |t| := by
    have hsmall_pos : 0 < a0 / Real.log |t| := div_pos ha0 hlog_pos
    have hinv_le : (σ - β)⁻¹ ≤ (a0 / Real.log |t|)⁻¹ := by
      have h :
          1 / (σ - β) ≤ 1 / (a0 / Real.log |t|) :=
        one_div_le_one_div_of_le hsmall_pos hgap_ge
      simpa [one_div] using h
    calc
      ‖(((σ - β : ℝ) : ℂ)⁻¹)‖ = (σ - β)⁻¹ := by
        rw [← Complex.ofReal_inv]
        exact Complex.norm_of_nonneg (inv_nonneg.mpr hgap_pos.le)
      _ ≤ (a0 / Real.log |t|)⁻¹ := hinv_le
      _ = (1 / a0) * Real.log |t| := by
        field_simp [ne_of_gt ha0, ne_of_gt hlog_pos]
  calc
    ‖logDeriv riemannZeta
        (((1 + a / Real.log |t| : ℝ) : ℂ) + I * t) -
        ((((1 + a / Real.log |t|) - β : ℝ) : ℂ)⁻¹)‖
    ...
    _ ≤ ‖logDeriv riemannZeta ((σ : ℂ) + I * t)‖ +
        ‖(((σ - β : ℝ) : ℂ)⁻¹)‖ := norm_sub_le _ _
    _ ≤ B₀ * Real.log |t| + (1 / a0) * Real.log |t| :=
        add_le_add hlogDeriv hprincipal
    _ = (B₀ + 1 / a0) * Real.log |t| := by ring

/-- Real-part form of
`exists_logDeriv_regular_part_sigmaOf_log_bound_of_absolute_convergence_uniform_of_le`. -/
lemma exists_re_neg_logDeriv_riemannZeta_sigmaOf_log_add_inv_le_of_absolute_convergence_uniform_of_le
    {a0 : ℝ} (ha0 : 0 < a0) :
    ∃ B : ℝ, 0 ≤ B ∧ ∀ a β t : ℝ, a0 ≤ a → 3 ≤ |t| → β < 1 →
      (-deriv riemannZeta
          (((1 + a / Real.log |t| : ℝ) : ℂ) + I * t) /
        ...
        1 / ((1 + a / Real.log |t|) - β) ≤
      B * Real.log |t| := by
  rcases exists_logDeriv_regular_part_sigmaOf_log_bound_of_absolute_convergence_uniform_of_le
      ha0 with ⟨B, hB, hbound⟩
  refine ⟨B, hB, ?_⟩
  intro a β t ha0a ht hβ
  let σ : ℝ := 1 + a / Real.log |t|
  ...
  have hnorm :
      ‖logDeriv riemannZeta ((σ : ℂ) + I * t) -
          (((σ - β : ℝ) : ℂ)⁻¹)‖ ≤
        B * Real.log |t| := by
    simpa [σ] using hbound a β t ha0a ht hβ
  have hregular :
      ‖-logDeriv riemannZeta ((σ : ℂ) + I * t) +
          (((σ - β : ℝ) : ℂ)⁻¹)‖ ≤
        B * Real.log |t| := by
...
/-- Fixed-multiplicity weak moving-line regular-part estimate, uniform for
all moving-line margins `a >= a0`.

The constant depends on the fixed lower margin `a0` and multiplicity `n`; this
still does not provide the uniform-in-`a -> 0` input needed for the classical
zero-free region. -/
lemma exists_norm_multiplicity_neg_logDeriv_regular_part_sigmaOf_log_bound_of_absolute_convergence_uniform_of_le
    {a0 : ℝ} (ha0 : 0 < a0) (n : ℕ) :
    ∃ B : ℝ, 0 ≤ B ∧ ∀ a β t : ℝ, a0 ≤ a → 3 ≤ |t| → β < 1 →
      ‖-logDeriv riemannZeta
          (((1 + a / Real.log |t| : ℝ) : ℂ) + I * t) +
  ...
  have hprincipal :
      ‖(n : ℂ) * (((σ - β : ℝ) : ℂ)⁻¹)‖ ≤
        ((n : ℝ) * (1 / a0)) * Real.log |t| := by
    calc
      ‖(n : ℂ) * (((σ - β : ℝ) : ℂ)⁻¹)‖ =
          (n : ℝ) * ‖(((σ - β : ℝ) : ℂ)⁻¹)‖ := by
            rw [norm_mul, Complex.norm_natCast]
      _ ≤ (n : ℝ) * ((1 / a0) * Real.log |t|) :=
          mul_le_mul_of_nonneg_left hprincipal_base (by positivity)
      _ = ((n : ℝ) * (1 / a0)) * Real.log |t| := by ring
  calc
    ‖-logDeriv riemannZeta
        (((1 + a / Real.log |t| : ℝ) : ℂ) + I * t) +
        (n : ℂ) * ((((1 + a / Real.log |t|) - β : ℝ) : ℂ)⁻¹)‖
    ...
    _ ≤ ‖-logDeriv riemannZeta ((σ : ℂ) + I * t)‖ +
        ‖(n : ℂ) * (((σ - β : ℝ) : ℂ)⁻¹)‖ := norm_add_le _ _
    _ ≤ B₀ * Real.log |t| + ((n : ℝ) * (1 / a0)) * Real.log |t| :=
        add_le_add hnegLogDeriv hprincipal
    _ = (B₀ + (n : ℝ) * (1 / a0)) * Real.log |t| := by ring

/-- Real-part form of
`exists_norm_multiplicity_neg_logDeriv_regular_part_sigmaOf_log_bound_of_absolute_convergence_uniform_of_le`. -/
lemma exists_re_neg_logDeriv_riemannZeta_sigmaOf_log_add_multiplicity_inv_le_of_absolute_convergence_uniform_of_le
    ...
    ∃ B : ℝ, 0 ≤ B ∧ ∀ a β t : ℝ, a0 ≤ a → 3 ≤ |t| → β < 1 →
      (-deriv riemannZeta
        ...
        1 / ((1 + a / Real.log |t|) - β) ≤
      B * Real.log |t| := by
  rcases exists_norm_multiplicity_neg_logDeriv_regular_part_sigmaOf_log_bound_of_absolute_convergence_uniform_of_le
      ha0 n with ⟨B, hB, hbound⟩
  refine ⟨B, hB, ?_⟩
  intro a β t ha0a ht hβ
  let σ : ℝ := 1 + a / Real.log |t|
  have ht2 : 2 ≤ |t| := by linarith
...
/-- Moving-line regular-part closure whose shifted input is supplied by the
named direct real-part vertical-strip bound for `Re(-ζ'/ζ)`.

This is the real-part analogue of
`classical_zero_free_region_of_sigmaOf_log_regular_part_norm_bound_and_vertical_logDeriv_norm_bound`:
it avoids passing through a norm estimate for the `σ+2it` term. -/
lemma classical_zero_free_region_of_sigmaOf_log_regular_part_norm_bound_and_ReNegDerivDivVerticalLogBound
    ...
    classical_zero_free_region := by
  rcases hvertical with ⟨hBvertical, _hTvertical, hvertical_bound⟩
  ...
  refine
    classical_zero_free_region_of_sigmaOf_log_regular_part_norm_bound_and_two_t_bound
      Bregular (2 * Bvertical) Tstar hBregular (by positivity) hTstar
      hregular_star ?_
  intro a t ha_pos ha_le_log2 ht
  let σ : ℝ := 1 + a / Real.log |t|
  have hσ_gt : 1 < σ := by
    simpa [σ] using sigmaOf_log_gt_one hTstar ha_pos ht
  have hσ_le : σ ≤ 2 := by
    simpa [σ] using sigmaOf_log_le_two hTstar ha_le_log2 ht
  have hTvertical_abs_t : Tvertical ≤ |t| :=
    (le_max_right Tregular Tvertical).trans ht
  have hTvertical_abs_two_t : Tvertical ≤ |(2 : ℝ) * t| := by
    calc
      Tvertical ≤ |t| := hTvertical_abs_t
      _ ≤ |(2 : ℝ) * t| := by
        rw [abs_mul]
        have ht_nonneg : 0 ≤ |t| := abs_nonneg t
        norm_num
  ...
  have hlog_two :
      Real.log |(2 : ℝ) * t| ≤ 2 * Real.log |t| :=
    log_abs_two_mul_le_two_log_abs (hTstar.trans ht)
  have hrewrite :
      ((σ : ℂ) + I * (((2 * t : ℝ) : ℂ))) =
        ((σ : ℂ) + 2 * I * t) := by
  ...
  have hbound :
      (-deriv riemannZeta ((σ : ℂ) + 2 * I * t) /
        ...
        Bvertical * Real.log |(2 : ℝ) * t| := by
    rw [← hrewrite]
    exact hvertical_bound σ (2 * t) hσ_gt.le hσ_le hTvertical_abs_two_t
  calc
    (-deriv riemannZeta
        ((1 + a / Real.log |t| : ℝ) + 2 * I * t) /
      riemannZeta ((1 + a / Real.log |t| : ℝ) + 2 * I * t)).re
        ≤ Bvertical * Real.log |(2 : ℝ) * t| := by
          simpa [σ] using hbound
    _ ≤ Bvertical * (2 * Real.log |t|) :=
        mul_le_mul_of_nonneg_left hlog_two hBvertical
    _ = (2 * Bvertical) * Real.log |t| := by ring

/-- Multiplicity-aware moving-line closure whose shifted input is supplied by
the named direct real-part vertical-strip bound for `Re(-ζ'/ζ)`. -/
lemma classical_zero_free_region_of_sigmaOf_log_multiplicity_regular_part_norm_bound_and_ReNegDerivDivVerticalLogBound
    (Bregular Tregular Bvertical Tvertical : ℝ)
    (hBregular : 0 ≤ Bregular) (hTregular : 2 ≤ Tregular)
    (hregular :
      ∀ a β t : ℝ, 0 < a → a ≤ Real.log 2 → Tregular ≤ |t| →
...
from the pole. -/
lemma analyticAt_logDeriv_riemannZeta_closedBall_sigmaOf_log_add_I_mul_of_radius_le_width_of_height_add_le
...
/-- Standard Borel/Jensen closed-ball analyticity around the shifted center
`σ + 2it`, with `σ = 1 + a / log |t|` and radius `a/(2 log |t|)`.

This is the analytic-domain input for the third term in the 3-4-1
combination when later local estimates are stated on canonical moving disks. -/
lemma analyticAt_logDeriv_riemannZeta_closedBall_sigmaOf_log_add_two_I_mul_borel_radius
    {T0 a t : ℝ} (hT0 : 2 ≤ T0) (ha : 0 < a) (ha_le : a ≤ Real.log 2)
    (ht : T0 ≤ |t|) :
  ...
  have hR : R ≤ a / Real.log |t| := by
    dsimp [R]
    linarith [hgeom.2.1]
  have hH_t : (1 : ℝ) + R ≤ |t| := by
    have hσ_le_two : 1 + a / Real.log |t| ≤ 2 :=
      sigmaOf_log_le_two hT0 ha_le ht
    have ht2 : 2 ≤ |t| := hT0.trans ht
    dsimp [R]
    linarith [hgeom.2.1, hσ_le_two, ht2]
  have ht_le_two_t : |t| ≤ |(2 : ℝ) * t| := by
    rw [abs_mul, abs_of_nonneg (by norm_num : (0 : ℝ) ≤ 2)]
    nlinarith [abs_nonneg t]
  have hH : (1 : ℝ) + R ≤ |(2 : ℝ) * t| := le_trans hH_t ht_le_two_t
  simpa [R] using
    analyticAt_logDeriv_riemannZeta_closedBall_sigmaOf_log_add_I_mul_height_of_radius_le_width_of_height_add_le
      hT0 ha ht (u := 2 * t) (R := R) (H := 1) hR
      (by norm_num : (0 : ℝ) < 1) hH
...
/-- Right-shifted regular-part zero-repulsion bridge with the center estimate
discharged by the Chebyshev moving-line bound.
...
the center `-logDeriv ζ(σ+r+it)` is controlled by the proved moving-line
estimate with parameter `a`, rather than by a fixed-margin estimate with
margin `r`.  The disk geometry still needs `1+r ≤ σ`; the analytic input still
is the real-part bound for the regular part on the Borel disk. -/
lemma exists_re_neg_logDeriv_riemannZeta_sigma_it_add_inv_right_shift_le_log_abs_of_affine_regularPart_re_le_half_radius_moving_line_center
    {a Are Bre r σ β t : ℝ}
  ...
  let C : ℝ := (2 * Are + 3 * (1 / r)) + 2 * (2 * Bre + 3 * Bcenter)
  have hA : 0 ≤ 2 * Are + 3 * (1 / r : ℝ) := by
    have hinv_nonneg : 0 ≤ (1 / r : ℝ) := by positivity
    nlinarith
  have hB : 0 ≤ 2 * Bre + 3 * Bcenter := by nlinarith [hBre, hBcenter]
  have hC : 0 ≤ C := by
    have hbase : 0 ≤ (2 * Are + 3 * (1 / r : ℝ)) + 2 * (2 * Bre + 3 * Bcenter) :=
      add_nonneg hA (mul_nonneg (by norm_num) hB)
    simpa [C] using hbase
  ...
  have hmain :=
    re_neg_logDeriv_riemannZeta_sigma_it_add_inv_right_shift_le_log_abs_of_affine_regularPart_re_le_half_radius
      (Are := Are) (Bre := Bre) (Acenter := 1 / r) (Bcenter := Bcenter)
      (r := r) (σ := σ) (β := β) (t := t)
      hr hσright hσr ht hA hB hM hdiff hlog hcenter hsub
  exact ⟨C, hC, by simpa [C] using hmain⟩
...
/-- Positivity of an affine logarithmic majorant in the full complex-height
scale.

This is the small bookkeeping fact needed to turn Borel-Carathéodory inputs
from `M > 0` into the natural assumptions `0 < A` and `0 <= B`. -/
lemma affine_log_norm_add_three_pos (z : ℂ) {A B : ℝ}
    (hA : 0 < A) (hB : 0 ≤ B) :
    0 < A + B * Real.log (‖z‖ + 3) := by
  have hlog_nonneg : 0 ≤ Real.log (‖z‖ + 3) := by
    apply Real.log_nonneg
...
/-- Positivity of an affine logarithmic majorant in the safe height scale
`log(|t|+3)`. -/
lemma affine_log_abs_add_three_pos (t : ℝ) {A B : ℝ}
    (hA : 0 < A) (hB : 0 ≤ B) :
    0 < A + B * Real.log (|t| + 3) := by
...
/-- Half-radius Borel-Carathéodory bound for `logDeriv ζ` with affine
full-height input, with the Borel positivity side-condition discharged by a
positive affine constant term. -/
lemma borelCaratheodory_logDeriv_riemannZeta_verticalRegion_of_one_le_re_of_affine_re_le_half_radius_of_pos_A
    {Are Bre Acenter Bcenter R σ t a b H : ℝ} {z : ℂ}
    (hAre : 0 < Are) (hBre_nonneg : 0 ≤ Bre)
    (ha₀ : 1 ≤ a) (hHpos : 0 < H)
    (hlog : ∀ w : ℂ, w ∈ verticalRegion a b H →
  ...
  borelCaratheodory_logDeriv_riemannZeta_verticalRegion_of_one_le_re_of_affine_re_le_half_radius
    ...
    (b := b) (H := H) (z := z)
    (affine_log_norm_add_three_pos (((σ : ℂ) + I * t)) hAre hBre_nonneg)
    ha₀ hHpos hlog hcenter ha hb hH hR hz_half
...
/-- Safe-height half-radius Borel-Carathéodory bound for `logDeriv ζ`, with
the Borel positivity side-condition discharged by `0 < Are` and `0 <= Bre`. -/
lemma borelCaratheodory_logDeriv_riemannZeta_verticalRegion_of_one_le_re_of_affine_log_abs_add_three_re_le_half_radius_of_pos_A
    {Are Bre Acenter Bcenter R σ t a b H : ℝ} {z : ℂ}
    (hσ : σ ∈ Set.Icc 1 2) (ht : 5 ≤ |t|)
    (hAre : 0 < Are) (hBre_nonneg : 0 ≤ Bre)
    (hBcenter_nonneg : 0 ≤ Bcenter)
  ...
  borelCaratheodory_logDeriv_riemannZeta_verticalRegion_of_one_le_re_of_affine_log_abs_add_three_re_le_half_radius
    ...
    (Bcenter := Bcenter) (R := R) (σ := σ) (t := t) (a := a)
    (b := b) (H := H) (z := z)
    hσ ht (affine_log_abs_add_three_pos t hAre hBre_nonneg)
    hBre_nonneg hBcenter_nonneg ha₀ hHpos hlog hcenter ha hb hH hR hz_half
...
affine full-height input, with `M > 0` discharged from `0 < Are`. -/
lemma borelCaratheodory_sub_logDeriv_riemannZeta_verticalRegion_of_one_le_re_of_affine_re_le_half_radius_of_pos_A
    {Are Bre R σ t a b H : ℝ} {z : ℂ}
    (hAre : 0 < Are) (hBre_nonneg : 0 ≤ Bre)
    (ha₀ : 1 ≤ a) (hHpos : 0 < H)
    (hlog : ∀ w : ℂ, w ∈ verticalRegion a b H →
  ...
  borelCaratheodory_sub_logDeriv_riemannZeta_verticalRegion_of_one_le_re_of_affine_re_le_half_radius
    ...
    (a := a) (b := b) (H := H) (z := z)
    (affine_log_norm_add_three_pos (((σ : ℂ) + I * t)) hAre hBre_nonneg)
    ha₀ hHpos hlog ha hb hH hR hz_half
...
/-- Safe-height oscillation Borel-Carathéodory bound for `logDeriv ζ`, with
the Borel positivity side-condition discharged by `0 < Are`. -/
lemma borelCaratheodory_sub_logDeriv_riemannZeta_verticalRegion_of_one_le_re_of_affine_log_abs_add_three_re_le_half_radius_of_pos_A
    {Are Bre R σ t a b H : ℝ} {z : ℂ}
    (hσ : σ ∈ Set.Icc 1 2) (ht : 5 ≤ |t|)
    (hAre : 0 < Are) (hBre_nonneg : 0 ≤ Bre)
    (ha₀ : 1 ≤ a) (hHpos : 0 < H)
  ...
  borelCaratheodory_sub_logDeriv_riemannZeta_verticalRegion_of_one_le_re_of_affine_log_abs_add_three_re_le_half_radius
    ...
    (a := a) (b := b) (H := H) (z := z)
    hσ ht (affine_log_abs_add_three_pos t hAre hBre_nonneg)
    hBre_nonneg ha₀ hHpos hlog ha hb hH hR hz_half
...
/-- Compact-plus-high-height zeta lower bounds feed the Cauchy/sphere bridge.

The fixed-radius boundary growth input is stated on the whole vertical region;
the center lower bound only has to be supplied at high height.  The missing
bounded-height lower bound is supplied by compactness and zero-freeness on
`Re(s) >= 1`. -/
lemma logDerivVerticalLogBound_of_compact_band_and_sphere_zeta_bound_high_height_zeta_lower_bound
    (H T R A B η : ℝ) (hH : 5 ≤ H) (hR : 0 < R) (hRlt : R < H)
  ...
  exact
    logDerivVerticalLogBound_of_sphere_zeta_bound_and_zeta_lower_bound_on_verticalRegion
      H R A B η' hH hR hRlt hA hB hη'_pos hsphere hzeta_all

/-- Real-part quotient version of
`logDerivVerticalLogBound_of_compact_band_and_sphere_zeta_bound_high_height_zeta_lower_bound`. -/
lemma reNegDerivDivVerticalLogBound_of_compact_band_and_sphere_zeta_bound_high_height_zeta_lower_bound
    (H T R A B η : ℝ) (hH : 5 ≤ H) (hR : 0 < R) (hRlt : R < H)
    (hA : 0 ≤ A) (hB : 0 ≤ B) (hη : 0 < η)
    (hsphere :
      ∀ c : ℂ, c ∈ verticalRegion 1 2 H →
...
/-- Final conditional assembly from regular-part input, all-height boundary
`ζ` growth on fixed-radius circles, and a high-height center lower bound for
`ζ`.  Compactness supplies the bounded-height lower-bound patch. -/
lemma classical_zero_free_region_of_exists_LogDerivRegularPartLogBound_and_compact_band_sphere_zeta_bound_high_height_zeta_lower_bound
    (hregular :
      ∃ Bregular Tregular : ℝ,
        LogDerivRegularPartLogBound Bregular Tregular)
    (H T R A B η : ℝ) (hH : 5 ≤ H) (hR : 0 < R) (hRlt : R < H)
...
/-- Multiplicity-aware version of
`classical_zero_free_region_of_exists_LogDerivRegularPartLogBound_and_compact_band_sphere_zeta_bound_high_height_zeta_lower_bound`. -/
lemma classical_zero_free_region_of_exists_MultiplicityLogDerivRegularPartLogBound_and_compact_band_sphere_zeta_bound_high_height_zeta_lower_bound
    (hregular :
      ∃ Bregular Tregular : ℝ,
        MultiplicityLogDerivRegularPartLogBound Bregular Tregular)
    (H T R A B η : ℝ) (hH : 5 ≤ H) (hR : 0 < R) (hRlt : R < H)
...
target.  The regular-part estimate is required only at
`sigma = 1 + a / log |t|`; the vertical logarithmic-derivative input is produced
from fixed-radius boundary growth for `zeta` and a high-height positive lower
bound for `zeta`, with the bounded-height lower-bound patch supplied by
compactness. -/
lemma classical_zero_free_region_of_sigmaOf_log_regular_part_norm_bound_and_compact_band_sphere_zeta_bound_high_height_zeta_lower_bound
...
/-- Moving-line regular-part assembly from high-height-only fixed-radius zeta
controls.

This is the high-height-input variant of
`classical_zero_free_region_of_sigmaOf_log_regular_part_norm_bound_and_compact_band_sphere_zeta_bound_high_height_zeta_lower_bound`:
both the sphere growth and center lower-bound hypotheses are required only for
large imaginary height; compactness supplies the bounded-height parts. -/
lemma classical_zero_free_region_of_sigmaOf_log_regular_part_norm_bound_and_compact_band_high_height_sphere_zeta_bound_zeta_lower_bound
    (Bregular Tregular H T R A B η : ℝ)
    (hBregular : 0 ≤ Bregular) (hTregular : 2 ≤ Tregular)
    (hH : 5 ≤ H) (hR : 0 < R) (hRlt : R < H)
    (hB : 0 ≤ B) (hη : 0 < η)
...
/-- Multiplicity-aware moving-line regular-part assembly from high-height-only
fixed-radius zeta controls. -/
lemma classical_zero_free_region_of_sigmaOf_log_multiplicity_regular_part_norm_bound_and_compact_band_high_height_sphere_zeta_bound_zeta_lower_bound
    (Bregular Tregular H T R A B η : ℝ)
    (hBregular : 0 ≤ Bregular) (hTregular : 2 ≤ Tregular)
    (hH : 5 ≤ H) (hR : 0 < R) (hRlt : R < H)
    (hB : 0 ≤ B) (hη : 0 < η)
...
/-- Patch the compact bounded-height `logDeriv ζ` norm bound with a future
high-height affine logarithmic estimate to cover the whole vertical region.

The compact part is already proved by differentiability and compactness.  The
remaining analytic input is the high-height estimate for `T <= |Im z|`, so this
lemma exposes the all-height affine bound needed by vertical-region handoffs
without claiming the future high-height estimate. -/
lemma exists_logDeriv_affine_log_norm_add_three_bound_on_verticalRegion_of_compact_band_and_high_height
    {H T A B : ℝ} (hH : 0 < H) (hB : 0 ≤ B)
    (hhigh : ∀ z : ℂ, z.re ∈ Set.Icc (1 : ℝ) 2 →
      T ≤ |z.im| →
        ‖logDeriv riemannZeta z‖ ≤ A + B * Real.log (‖z‖ + 3)) :
...
/-- Compact-plus-high-height affine `logDeriv ζ` bounds feed directly into the
named vertical logarithmic-derivative interface. -/
lemma logDerivVerticalLogBound_of_compact_band_and_high_height_affine_log_norm_add_three_bound
...
/-- Direct real-part quotient version of
`exists_logDeriv_affine_log_norm_add_three_bound_on_verticalRegion_of_compact_band_and_high_height`.

This patches the proved compact bounded-height control of `Re(-ζ'/ζ)` with a
future high-height affine estimate in the exact real-part quotient convention
used by the final zero-free-region bridge. -/
lemma exists_reNegDerivDiv_affine_log_norm_add_three_bound_on_verticalRegion_of_compact_band_and_high_height
    {H T A B : ℝ} (hH : 0 < H) (hB : 0 ≤ B)
    (hhigh : ∀ z : ℂ, z.re ∈ Set.Icc (1 : ℝ) 2 →
      T ≤ |z.im| →
        (-deriv riemannZeta z / riemannZeta z).re ≤
  ...
  have hlog_nonneg : 0 ≤ Real.log (‖z‖ + 3) := by
    ...
    nlinarith [norm_nonneg z]
  by_cases hle : |z.im| ≤ T
  · have hcompact_norm : ‖-logDeriv riemannZeta z‖ ≤ C :=
      hcompact z hz.1 hz.2 hle
    have hcompact_re :
        (-deriv riemannZeta z / riemannZeta z).re ≤ C := by
      calc
        (-deriv riemannZeta z / riemannZeta z).re
...
/-- Compact-plus-high-height affine `Re(-ζ'/ζ)` bounds feed directly into
the named real-part vertical logarithmic-derivative interface. -/
lemma reNegDerivDivVerticalLogBound_of_compact_band_and_high_height_affine_log_norm_add_three_bound
    (H T A B : ℝ) (hH : 5 ≤ H) (hB : 0 ≤ B)
    (hhigh : ∀ z : ℂ, z.re ∈ Set.Icc (1 : ℝ) 2 →
      T ≤ |z.im| →
        (-deriv riemannZeta z / riemannZeta z).re ≤
...
/-- Final named-interface assembly from regular-part input plus a high-height
coordinate affine logarithmic bound for `logDeriv ζ`.

This is the target-shaped version of
`classical_zero_free_region_of_exists_LogDerivRegularPartLogBound_and_compact_band_high_height_logDeriv_affine_bound`
when the future analytic estimate is stated directly as
`A + B * log(|t| + 3)` on points `σ + it`. -/
lemma classical_zero_free_region_of_exists_LogDerivRegularPartLogBound_and_high_height_affine_log_abs_add_three_logDeriv_bound
    (hregular :
      ∃ Bregular Tregular : ℝ,
        LogDerivRegularPartLogBound Bregular Tregular)
    (T0 A B : ℝ) (hT0 : 3 ≤ T0) (hA : 0 ≤ A) (hB : 0 ≤ B)
...
/-- Multiplicity-aware final named-interface assembly from regular-part input
plus a high-height coordinate affine logarithmic bound for `logDeriv ζ`. -/
lemma classical_zero_free_region_of_exists_MultiplicityLogDerivRegularPartLogBound_and_high_height_affine_log_abs_add_three_logDeriv_bound
    (hregular :
      ∃ Bregular Tregular : ℝ,
        MultiplicityLogDerivRegularPartLogBound Bregular Tregular)
    (T0 A B : ℝ) (hT0 : 3 ≤ T0) (hA : 0 ≤ A) (hB : 0 ≤ B)
...
/-- Moving-line regular-part assembly from a high-height coordinate affine
logarithmic bound for `logDeriv ζ`.

This accepts the boundary-strip estimate in the most common analytic form,
`‖logDeriv ζ(σ+it)‖ ≤ A + B log(|t|+3)`, and combines it with the moving-line
regular-part input. -/
lemma classical_zero_free_region_of_sigmaOf_log_regular_part_norm_bound_and_high_height_affine_log_abs_add_three_logDeriv_bound
...
/-- Multiplicity-aware moving-line assembly from a high-height coordinate
affine logarithmic bound for `logDeriv ζ`. -/
lemma classical_zero_free_region_of_sigmaOf_log_multiplicity_regular_part_norm_bound_and_high_height_affine_log_abs_add_three_logDeriv_bound
    (Bregular Tregular T0 A B : ℝ)
    (hBregular : 0 ≤ Bregular) (hTregular : 2 ≤ Tregular)
    (hT0 : 3 ≤ T0) (hA : 0 ≤ A) (hB : 0 ≤ B)
    (hregular :
...
/-- Moving-line regular-part assembly from a high-height coordinate affine
real-part bound for `Re(-ζ'/ζ)`.

This is the direct real-part analogue of
`classical_zero_free_region_of_sigmaOf_log_regular_part_norm_bound_and_high_height_affine_log_abs_add_three_logDeriv_bound`.
It is the expected target shape when a future Borel/Jensen argument already
produces the 3-4-1 sign convention rather than a norm bound. -/
lemma classical_zero_free_region_of_sigmaOf_log_regular_part_norm_bound_and_high_height_affine_re_log_abs_add_three_bound
    (Bregular Tregular T0 A B : ℝ)
    (hBregular : 0 ≤ Bregular) (hTregular : 2 ≤ Tregular)
    (hT0 : 3 ≤ T0) (hA : 0 ≤ A) (hB : 0 ≤ B)
    (hregular :
  ...
  rcases
    ...
    ⟨Bvertical, Tvertical, hvertical⟩
  exact
    classical_zero_free_region_of_sigmaOf_log_regular_part_norm_bound_and_ReNegDerivDivVerticalLogBound
      Bregular Tregular Bvertical Tvertical hBregular hTregular
      hregular hvertical

/-- Multiplicity-aware moving-line assembly from a high-height coordinate
affine real-part bound for `Re(-ζ'/ζ)`. -/
lemma classical_zero_free_region_of_sigmaOf_log_multiplicity_regular_part_norm_bound_and_high_height_affine_re_log_abs_add_three_bound
    (Bregular Tregular T0 A B : ℝ)
    (hBregular : 0 ≤ Bregular) (hTregular : 2 ≤ Tregular)
    (hT0 : 3 ≤ T0) (hA : 0 ≤ A) (hB : 0 ≤ B)
    (hregular :
  ...
  rcases
    ...
    ⟨Bvertical, Tvertical, hvertical⟩
  exact
    classical_zero_free_region_of_sigmaOf_log_multiplicity_regular_part_norm_bound_and_ReNegDerivDivVerticalLogBound
      Bregular Tregular Bvertical Tvertical hBregular hTregular
      hregular hvertical
...
/-- High-height-only compact-patched Cauchy/sphere primitive-input version of
the 3-4-1 real-part shifted-pair handoff.
...
both the boundary-circle growth and the center lower bound for `ζ` are assumed
only at high height.  Compactness supplies the bounded-height circles and
bounded-height center lower bounds. -/
lemma exists_re_neg_deriv_div_riemannZeta_shift_pair_vertical_log_bound_of_compact_band_high_height_sphere_zeta_bound_zeta_lower_bound
    (H T R A B eta : ℝ) (hH : 5 ≤ H) (hR : 0 < R) (hRlt : R < H)
    (hB : 0 ≤ B) (heta : 0 < eta)
    (hsphereHigh :
      ∀ c : ℂ, c.re ∈ Set.Icc (1 : ℝ) 2 → T ≤ |c.im| →
  ...
  rcases
      reNegDerivDivVerticalLogBound_of_compact_band_and_high_height_sphere_zeta_bound_zeta_lower_bound
        H T R A B eta hH hR hRlt hB heta hsphereHigh hzetaHigh with
    ⟨C, T0, hre⟩
  exact
    exists_re_neg_deriv_div_riemannZeta_shift_pair_vertical_log_bound_of_ReNegDerivDivVerticalLogBound
      (B := C) (T0 := T0) hre
...
BTY bridges above.  It keeps the input in the convention used by the detector
inequality itself. -/
lemma exists_log_deriv_zeta_bty_detector_one_lower_bound_of_fixed_margin_center_and_re_high_height_log_abs_bound
    {ε T0 B : ℝ} (hε : 0 < ε) (hB : 0 ≤ B)
    (hhigh :
      ∀ σ t : ℝ, 1 ≤ σ → σ ≤ 2 → T0 ≤ |t| →
        (-deriv riemannZeta ((σ : ℂ) + I * t) /
      ...
      ∀ σ t : ℝ, 1 + ε ≤ σ → σ ≤ 2 → T0' ≤ |t| →
        (-deriv riemannZeta ((σ : ℂ) + I * t) /
          ...
          - (∑ k ∈ btyDetectorSupport.erase 1, btyDetectorCoeff k *
              (if k = 0 then K else C * Real.log (17 * (|t| + 3)))) /
            btyDetectorCoeff 1 := by
  rcases reNegDerivDivVerticalLogBound_of_high_height_log_abs_bound
      (T0 := T0) (B := B) hB hhigh with
    ⟨C, T0', hreal⟩
  rcases exists_log_deriv_zeta_bty_detector_one_lower_bound_of_fixed_margin_center_and_ReNegDerivDivVerticalLogBound
      (ε := ε) (C := C) (T0 := T0') hε hreal with
    ⟨K, hK, hbound⟩
  exact ⟨K, C, T0', hK, hreal, hbound⟩

/-- Simplified fixed-margin BTY handoff fed directly by a future high-height
`B * log |t|` bound for `Re(-ζ'/ζ)`. -/
lemma exists_log_deriv_zeta_bty_detector_one_lower_bound_of_fixed_margin_center_and_re_high_height_log_abs_bound_simplified
    {ε T0 B : ℝ} (hε : 0 < ε) (hB : 0 ≤ B)
    (hhigh :
      ∀ σ t : ℝ, 1 ≤ σ → σ ≤ 2 → T0 ≤ |t| →
        (-deriv riemannZeta ((σ : ℂ) + I * t) /
  ...
  rcases reNegDerivDivVerticalLogBound_of_high_height_log_abs_bound
      (T0 := T0) (B := B) hB hhigh with
    ⟨C, T0', hreal⟩
  rcases exists_log_deriv_zeta_bty_detector_one_lower_bound_of_fixed_margin_center_and_ReNegDerivDivVerticalLogBound_simplified
      (ε := ε) (C := C) (T0 := T0') hε hreal with
    ⟨K, hK, hbound⟩
  exact ⟨K, C, T0', hK, hreal, hbound⟩
...
/-- Signed half-radius Borel-Carathéodory bound for `-logDeriv ζ` with affine
full-height input, with `M > 0` discharged from `0 < Are`. -/
lemma borelCaratheodory_neg_logDeriv_riemannZeta_verticalRegion_of_one_le_re_of_affine_re_le_half_radius_of_pos_A
    {Are Bre Acenter Bcenter R σ t a b H : ℝ} {z : ℂ}
    (hAre : 0 < Are) (hBre_nonneg : 0 ≤ Bre)
    (ha₀ : 1 ≤ a) (hHpos : 0 < H)
    (hlog : ∀ w : ℂ, w ∈ verticalRegion a b H →
...
/-- Signed safe-height half-radius Borel-Carathéodory bound for `-logDeriv ζ`,
with `M > 0` discharged from `0 < Are`. -/
lemma borelCaratheodory_neg_logDeriv_riemannZeta_verticalRegion_of_one_le_re_of_affine_log_abs_add_three_re_le_half_radius_of_pos_A
    {Are Bre Acenter Bcenter R σ t a b H : ℝ} {z : ℂ}
    (hσ : σ ∈ Set.Icc 1 2) (ht : 5 ≤ |t|)
    (hAre : 0 < Are) (hBre_nonneg : 0 ≤ Bre)
    (hBcenter_nonneg : 0 ≤ Bcenter)
...
/-- Signed oscillation Borel-Carathéodory bound with affine full-height input,
with `M > 0` discharged from `0 < Are`. -/
lemma borelCaratheodory_sub_neg_logDeriv_riemannZeta_verticalRegion_of_one_le_re_of_affine_re_le_half_radius_of_pos_A
    {Are Bre R σ t a b H : ℝ} {z : ℂ}
    (hAre : 0 < Are) (hBre_nonneg : 0 ≤ Bre)
    (ha₀ : 1 ≤ a) (hHpos : 0 < H)
    (hlog : ∀ w : ℂ, w ∈ verticalRegion a b H →
...
/-- Signed safe-height oscillation Borel-Carathéodory bound, with the Borel
positivity side-condition discharged by `0 < Are`. -/
lemma borelCaratheodory_sub_neg_logDeriv_riemannZeta_verticalRegion_of_one_le_re_of_affine_log_abs_add_three_re_le_half_radius_of_pos_A
    {Are Bre R σ t a b H : ℝ} {z : ℂ}
    (hσ : σ ∈ Set.Icc 1 2) (ht : 5 ≤ |t|)
    (hAre : 0 < Are) (hBre_nonneg : 0 ≤ Bre)
    (ha₀ : 1 ≤ a) (hHpos : 0 < H)
...
The center is uniformly separated from zero by
`one_third_le_norm_riemannZeta_of_two_le_re`, so Jensen's center term costs at
most `log 3`.  This removes a separate center-value hypothesis from the
high-height zero-counting route; the remaining analytic input is the boundary
growth estimate `hsphere`. -/
lemma jensen_zero_mass_riemannZeta_two_add_I_mul_le_log_bound
  ...
  have hc_ne_zero : riemannZeta c ≠ 0 :=
    riemannZeta_ne_zero_of_one_le_re (by simp [hc_re])
  have hcenter_lower : (1 / 3 : ℝ) ≤ ‖riemannZeta c‖ :=
    one_third_le_norm_riemannZeta_of_two_le_re c (by simp [hc_re])
  ...
  have hcircle_integrable :
      CircleIntegrable (Real.log ‖riemannZeta ·‖) c R := by
    apply circleIntegrable_log_norm_meromorphicOn
    intro z hz
    exact meromorphicOn_riemannZeta_closedBall c R z
      (sphere_subset_closedBall (by simpa [abs_of_pos hR] using hz))
  have hcircle_le :
      circleAverage (Real.log ‖riemannZeta ·‖) c R ≤ Real.log M := by
    refine circleAverage_mono_on_of_le_circle hcircle_integrable ?_
    intro z hz
    by_cases hzeta : riemannZeta z = 0
    · simp [hzeta, Real.log_nonneg hM]
    · exact Real.log_le_log (norm_pos_iff.mpr hzeta)
        (hsphere z (by simpa [c, abs_of_pos hR] using hz))
  have hdiv_center : divisor riemannZeta (closedBall c R) c = 0 := by
    rw [MeromorphicOn.divisor_apply (meromorphicOn_riemannZeta_closedBall c R)
      (by simp [hR.le])]
    have hnormal := hc_analytic.meromorphicNFAt
    rw [(hnormal.meromorphicOrderAt_eq_zero_iff).2 hc_ne_zero]
    simp
...
/-- Jensen's weighted mass bound controls the total multiplicity of zeta
zeros in every strictly smaller concentric disk.

The factor `log (R / r)` is the minimum Jensen weight contributed by a zero
in the inner disk.  The high-height condition keeps the outer disk away from
the pole at `1`, hence its zeta divisor is nonnegative. -/
lemma jensen_inner_zero_multiplicity_count_riemannZeta_two_add_I_mul_le_log_bound
    {r R t M : ℝ} (hr : 0 < r) (hrR : r < R) (hheight : R < |t|)
    (hM : 1 ≤ M)
    (hsphere : ∀ z : ℂ,
      z ∈ sphere ((2 : ℂ) + I * t) R → ‖riemannZeta z‖ ≤ M) :
    Real.log (R / r) *
        (∑ᶠ u ∈ (closedBall ((2 : ℂ) + I * t) r : Set ℂ),
          (divisor riemannZeta
            (closedBall ((2 : ℂ) + I * t) R) u : ℝ)) ≤
      Real.log M + Real.log 3 := by
  let c : ℂ := (2 : ℂ) + I * t
  let D := divisor riemannZeta (closedBall c R)
  have hR : 0 < R := hr.trans hrR
  have havoid : ∀ z : ℂ, z ∈ closedBall c R → z ≠ 1 := by
    intro z hz hz_one
    subst z
    have hdist : dist (1 : ℂ) c ≤ R := by
      simpa using hz
  ...
  have hpoint : ∀ u : ℂ,
    ...
    have hnorm_outer : ‖c - u‖ ≤ R := by
      simpa [mem_closedBall, dist_eq_norm, norm_sub_rev] using hu_outer
    by_cases hu_inner : u ∈ closedBall c r
    · have hnorm_inner : ‖c - u‖ ≤ r := by
        simpa [mem_closedBall, dist_eq_norm, norm_sub_rev] using hu_inner
  ...
  have hleft_support :
    ...
    simp [hDu_zero] at hu
  have hright_support :
      (fun u : ℂ => (D u : ℝ) * Real.log (R * ‖c - u‖⁻¹)).support ⊆
        hD_finite.toFinset := by
    intro u hu
    apply hD_finite.mem_toFinset.mpr
    by_contra hDu
  ...
  have hmass :=
    jensen_zero_mass_riemannZeta_two_add_I_mul_le_log_bound
      (R := R) (t := t) (M := M) hR hM hsphere
  calc
    Real.log (R / r) *
        (∑ᶠ u ∈ (closedBall ((2 : ℂ) + I * t) r : Set ℂ),
          (divisor riemannZeta
