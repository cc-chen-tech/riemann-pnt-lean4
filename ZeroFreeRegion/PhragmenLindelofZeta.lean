...
/-- Unconditional regular-part estimate on the full fixed disk reaching
`Re(s) = 1`.  All Jensen/Borel inputs are discharged by the polynomial zeta
growth theorem above; no vertical-growth premise remains. -/
...
/-- Uniform logarithmic regular-part estimate in the boundary disk.  This is
the unconditional `O(log |t|)` input needed by zero-repulsion arguments: the
only singular terms left are the explicitly displayed local zeta-zero
principal parts. -/
