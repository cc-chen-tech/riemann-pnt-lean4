...
contains no syntactic \texttt{sorry} occurrences.  Secondary verified results
include a pointwise-in-$x$ quantitative Riemann--von Mangoldt formula using the
midpoint Chebyshev function and finite-height truncations, proved
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{PNT infrastructure:} proved equivalence wrappers for the
    ...
    compose with a future proof of \(\psi(x)\sim x\); RH-scale use of the
    explicit formula still requires uniform control of its constant as
    \(x\) varies.
...
is pointwise in \(x\); it does not yet provide the uniform \(x\)-dependence
needed to derive RH-scale prime-counting errors.
